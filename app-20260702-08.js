(function () {
  ...
  function bookingWorkflowButtons(row, options = {}) {
    ...
    return [
      ...
      done ? `<button class="secondary" data-price-basis-booking="${escapeHtml(row.id)}" type="button">Fakturagrunnlag</button>` : "",
  ...
  function bookingIsSettledForPlanning(row) {
    ...
    if (!billableJobType(bookingDisplayType(row))) return true;
  ...
  function openReminderDialog(customerId = "") {
    ...
    if (el.reminderDueTime) el.reminderDueTime.value = "";
  ...
  function orderDetailActionsHtml(order, customer, primaryBooking, effectiveBilling, linkedJob) {
    ...
    if (primaryBooking) {
      ...
      if (done) more.push(`<button class="secondary" data-price-basis-booking="${escapeHtml(primaryBooking.id)}" type="button">Fakturagrunnlag</button>`);
  ...
  function jobCard(row, editable, options = {}) {
    ...
    card.innerHTML = `
      ...
      <div class="job-actions">
        ...
        ${editable && isAdmin() && done ? `<button data-price-basis-booking="${escapeHtml(row.id)}" type="button" title="Rediger prisgrunnlag og tillegg før eller etter faktura.">Fakturagrunnlag</button>` : ""}
