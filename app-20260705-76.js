(function () {
  ...
  function leadNextActionHtml(entry, customer, realCustomer, status, leadTarget) {
    ...
    if (realCustomer && status === "followup") {
      primaryActions.push(`<button class="order-primary" data-lead-set-status="needs_offer" data-lead-status-customer="${escapeHtml(leadTarget)}" type="button" title="Flytt saken til tilbudskøen når du har nok info til å lage tilbud.">Klar for tilbud</button>`);
      secondaryActions.push(`<button class="secondary" data-book-customer="${escapeHtml(key)}" data-book-type="befaring" type="button" title="Legg befaring eller annen oppfølging i kalenderen.">Book befaring</button>`);
    }
    if (realCustomer && status === "needs_offer") {
      ...
      secondaryActions.push(`<button class="secondary" data-lead-set-status="offer_sent" data-lead-status-customer="${escapeHtml(leadTarget)}" type="button" title="Brukes bare når tilbudet faktisk er sendt utenfor CRM eller bekreftet sendt.">Marker sendt</button>`);
    }
  ...
  function renderLeadDetail() {
    ...
    el.leadDetail.innerHTML = `
      ...
      <section class="detail-section">
        <h3>Status</h3>
        <details>
          <summary>Manuelle statusvalg</summary>
