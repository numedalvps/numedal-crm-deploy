(function () {
  ...
  function cleanLeadOrderMatchTerm(value) {
    const term = String(value || "")
      ...
      .replace(/\b(sum tilbudslinjer|tilbudslinjer|lead|phone|new installation)\b/gi, " ")
      .replace(/\s+/g, " ")
      .trim();
  ...
  function leadStatusProductSummaryFromLines(text) {
    const labels = String(text || "")
      ...
      .map((line) => line.replace(/^Sum tilbudslinjer:.*/i, "").trim())
  ...
  function offerLinesFromText(text) {
    const match = String(text || "").match(/\[Tilbudslinjer\]([\s\S]*?)\[\/Tilbudslinjer\]/i);
    if (!match?.[1]) return "";
    return match[1]
      .split(/\r?\n/)
      .map((line) => line.trim())
      .filter((line) => line && !/^Sum tilbudslinjer\s*:/i.test(line))
      .join("\n");
  }
  ...
  function priceBasisLineUnitPrice(line) {
    const match = String(line || "").match(/\bx\s*([\d\s]+(?:,\d{1,2})?)\s*(?:,-|kr|inkl|=)/i);
  ...
  function addCompletionPriceLine() {
    ...
    if (!item || item.id === "heatpump_custom") {
      ...
      if (!Number.isFinite(quantity) || quantity <= 0) throw new Error("Antall må være større enn 0.");
      line = `- ${label}: ${formatJobPriceNumber(quantity)} stk - pris avtales/legges inn`;
    } else {
