(function () {
  ...
  function showManualCopyFallback(text) {
    ...
    wrap.innerHTML = `
      ...
      <p>Hold inne i feltet og velg Kopier. Trykk Marker alt hvis mobilen mister markeringen.</p>
  ...
  async function copyPhone(button) {
    ...
    if (!phone) return;
    const oldText = button.textContent;
    const copied = await copyTextToClipboard(phone, { manualFallback: true });
    button.textContent = copied ? "Kopiert" : "Kopier manuelt";
    setSyncStatus(copied ? `Telefonnummer kopiert: ${phone}` : manualCopyStatus("Telefonnummeret"), copied ? "ok" : "");
    setTimeout(() => {
      button.textContent = oldText;
    }, 1400);
  }
  ...
  async function copyEmail(button) {
    ...
    if (!email) return;
    const oldText = button.textContent;
    const copied = await copyTextToClipboard(email, { manualFallback: true });
    button.textContent = copied ? "Kopiert" : "Kopier manuelt";
    setSyncStatus(copied ? `E-post kopiert: ${email}` : manualCopyStatus("E-postadressen"), copied ? "ok" : "");
    setTimeout(() => {
      button.textContent = oldText;
    }, 1400);
  }
