(function () {
  ...
  function offerActivityDetails(activity, customer = null) {
    ...
    const total = metadataTotal > 0 ? metadataTotal : jobPriceBasisTotal(lines || text);
  ...
  function offerLinesBlock(linesText) {
    ...
    return [
      ...
      "Prisgrunnlag:",
      ...lines,
      total > 0 ? `Sum prisgrunnlag: ${formatJobPriceAmount(total)} inkl. mva` : "",
  ...
  function offerActivityDataFromPayload(payload = {}) {
    ...
    const product = leadStatusProductSummaryFromLines(lines)
      ...
      || "Tilbud";
    const total = jobPriceBasisTotal(lines || payload.text || "");
