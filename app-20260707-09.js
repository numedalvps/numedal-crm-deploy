(function () {
    ...
    const openHtml = rows.length ? `
      <div class="section-head compact">
        <div>
          ...
          <p title="Dette er rå skjema fra nettsiden. Første trygge steg er kundekort, deretter tilbud, service eller jobb.">Første steg er kundekort. Deretter velger du tilbud, service eller jobb.</p>
        </div>
        <strong>${rows.length.toLocaleString("nb-NO")}</strong>
      </div>
  ...
  function websiteSubmissionWorkflowHint(row, duplicateCount = 0) {
    ...
    return {
      kind: duplicateCount ? "lead duplicate" : "lead",
      label: "Forslag: Tilbud / oppfølging",
      detail: duplicateCount
      ...
      <section class="detail-section lead-offer-section">
        ...
        <div class="lead-offer-builder">
          <div class="lead-offer-builder-head">
            ...
            <button class="secondary" data-insert-offer-lines="${escapeHtml(target)}" type="button" title="Legger tilbudsgrunnlag og prislinjer inn i teksten under.">Legg i tekst</button>
          </div>
  ...
  function addOfferLine(target) {
    ...
    setSyncStatus("Prislinje lagt til. Trykk Legg i tekst når tilbudet er klart.", "ok");
  }
  ...
  el.leadDetail?.addEventListener("click", (event) => {
    ...
    if (statusButton) {
      runBusyButton(statusButton, () => changeLeadStatusFromUi(statusButton.dataset.leadStatusCustomer, statusButton.dataset.leadSetStatus), "Klarte ikke endre henvendelsesstatus.");
      return;
    }
    ...
    if (createCustomer) {
      runBusyButton(createCustomer, () => createCustomerFromLead(createCustomer.dataset.createCustomerFromLead), "Klarte ikke opprette kundekort fra henvendelse.");
      return;
    }
    const newLead = event.target.closest("[data-new-lead-existing-customer]");
    if (newLead) {
