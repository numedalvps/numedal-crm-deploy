(function () {
  ...
  const jobPriceItems = [
    { id: "standard_installation", label: "Standard montering kundens/brukt pumpe", unit: "stk", price: 6990, defaultQty: 1 },
    { id: "technician_hour", label: "Timepris arbeid", unit: "time", price: 1150, defaultQty: 1 },
    { id: "old_pump_removal", label: "Demontering gammel pumpe inkl. gasstømming", unit: "stk", price: 1990, defaultQty: 1 },
    { id: "no_show", label: "Bomtur / avbrutt installasjon", unit: "stk", price: 1600, defaultQty: 1 },
    { id: "wall_drill_simple", label: "Enkel murboring", unit: "stk", price: 990, defaultQty: 1 },
    { id: "trollflex", label: "Trollflex rørgjennomføring", unit: "stk", price: 800, defaultQty: 1 },
    { id: "extra_pipe", label: "Ekstra rør og signal-/strømkabel", unit: "m", price: 650, defaultQty: 1 },
    { id: "connection_3m", label: "Tilslutningskabel 3 meter", unit: "stk", price: 295, defaultQty: 1 },
    { id: "connection_7m", label: "Tilslutningskabel 7 meter", unit: "stk", price: 495, defaultQty: 1 },
    { id: "connection_10m", label: "Tilslutningskabel 10 meter", unit: "stk", price: 790, defaultQty: 1 },
    { id: "signal_cable_joint", label: "Signalkabel skjøt", unit: "stk", price: 300, defaultQty: 1 },
    { id: "gas_pipe_joint", label: "Gassrør skjøt 1/4-3/8 komplett", unit: "stk", price: 550, defaultQty: 1 },
    { id: "blind_sleeves", label: "Blindehylser 1/4-3/8", unit: "stk", price: 300, defaultQty: 1 },
    { id: "extra_channel", label: "PVC kanaler", unit: "m", price: 300, defaultQty: 1 },
    { id: "pvc_bend", label: "PVC 90° bend", unit: "stk", price: 180, defaultQty: 1 },
    { id: "pvc_wall_cap", label: "PVC vegglokk", unit: "stk", price: 150, defaultQty: 1 },
    { id: "pvc_flex", label: "PVC flexrør 600 mm", unit: "stk", price: 300, defaultQty: 1 },
    { id: "rubber_dampers", label: "4 stk. standard gummidempere", unit: "sett", price: 500, defaultQty: 1 },
    { id: "masonry_brackets", label: "Murbraketter 2 stk. vinkler", unit: "sett", price: 700, defaultQty: 1 },
    { id: "spring_dampers", label: "4 stk. fjærdempere", unit: "sett", price: 990, defaultQty: 1 },
    { id: "damped_wall_stand", label: "Dempet veggstativ", unit: "stk", price: 750, defaultQty: 1 },
    { id: "wood_wall_bracket", label: "Dempet veggbrakett med fjærer for trevegg", unit: "stk", price: 1750, defaultQty: 1 },
    { id: "ground_stand", label: "Markkonsoll", unit: "stk", price: 1870, defaultQty: 1 },
    { id: "low_wall_bracket", label: "Spesialbrakett lav murvegg", unit: "stk", price: 990, defaultQty: 1 },
    { id: "stone_slabs", label: "4 stk. steinheller til bakkestativ", unit: "sett", price: 990, defaultQty: 1 },
    { id: "heatpump_roof_white", label: "Varmepumpetak hvit", unit: "stk", price: 1990, defaultQty: 1 },
    { id: "heatpump_roof_black", label: "Varmepumpetak sort", unit: "stk", price: 1990, defaultQty: 1 },
    { id: "drip_pan_heated", label: "Dryppanne komplett kit inkl. varmekabel", unit: "stk", price: 3990, defaultQty: 1 },
    { id: "heatpump_house_white", label: "VPS90 hvit varmepumpehus", unit: "stk", price: 3990, defaultQty: 1 },
    { id: "heatpump_house_black", label: "VPS90-K sort varmepumpehus", unit: "stk", price: 4290, defaultQty: 1 },
    { id: "r32_refrigerant", label: "Ekstra kjølemedium R32", unit: "g", price: 6, defaultQty: 100 },
    { id: "r410a_refrigerant", label: "Ekstra kjølemedium R410A", unit: "g", price: 7.5, defaultQty: 100 },
  ...
  const leadStatuses = {
    ...
    lost: {
      ...
      help: "Henvendelsen er avsluttet uten salg. Beholdes for historikk og eventuell senere serviceforespørsel.",
    },
  };
  ...
  const aiRegistrationTypes = {
    lead: {
      ...
      eventType: "Hurtigregistrering - henvendelse",
  ...
  function leadStatusLabel(status) {
    return leadStatuses[status]?.label || "Henvendelse";
  }
  ...
  async function setLeadStatus(customerId, status) {
    ...
    if (dbLead?.id && store.updateLead) {
      ...
      await saveServiceEvent(customer, {
        ...
        event_type: "Henvendelsesstatus",
        note: `Status satt til: ${leadStatusLabel(status)}.`,
      });
    ...
    const syncedLead = await syncLeadRecord(saved || customer, status, `Status satt til: ${leadStatusLabel(status)}.`);
    await saveServiceEvent(customer, {
      event_date: isoDate(new Date()),
      event_type: "Henvendelsesstatus",
      note: `Status satt til: ${leadStatusLabel(status)}.`,
    ...
    renderAll();
    setView("leads");
    setSyncStatus(status ? `Henvendelse satt til: ${leadStatusLabel(status)}.` : "Henvendelsesstatus fjernet.", "ok");
  }
  ...
  async function saveLeadStatusActivity(lead, status, customer = null) {
    await saveActivityRecord({
      ...
      summary: `Henvendelse: ${leadStatusLabel(status)}`,
      body: `Status satt til: ${leadStatusLabel(status)}.`,
  ...
  function leadStatusControlHtml(customer, statusOverride = "", targetKey = "") {
    ...
    return `
      <label class="lead-status-control">Status
        <select data-lead-status-customer="${escapeHtml(key)}" title="Velg neste steg for denne henvendelsen.">
          ${options}
        </select>
      </label>
    `;
  ...
  function leadManualStatusActionsHtml(leadTarget, currentStatus, realCustomer) {
    ...
    return rows.map(([value, label, title]) => {
      ...
      + (realCustomer && currentStatus !== "followup"
        ? `<button class="secondary" data-lead-set-status="followup" data-lead-status-customer="${escapeHtml(leadTarget)}" type="button" title="Flytt saken tilbake til nye henvendelser hvis den må avklares på nytt.">Gjenåpne</button>`
        : "");
  }
    ...
    el.aiRegistrationDraft.innerHTML = `
      ...
      <div class="ai-draft-grid">
        <div class="ai-draft-form">
          ...
          <label>Tags
            <input id="aiRegistrationTags" data-ai-field type="text" value="${escapeHtml(aiRegistrationDraft.tags)}" placeholder="Henvendelse; Service; Blåseisolering" />
          </label>
          ...
          <div class="ai-save-row">
            <button id="aiRegistrationInboxButton" class="secondary" type="button" title="Legg forslaget i CRM-innboks uten å opprette kunde eller henvendelse ennå.">Lagre i CRM-innboks</button>
            <button id="aiRegistrationSaveButton" type="button">Lagre godkjent forslag</button>
            <span class="compact-help" title="CRM-innboks er bare utkast. Lagre godkjent forslag oppretter eller kobler kunde, henvendelse eller historikk.">?</span>
          </div>
        </div>
  ...
  async function saveAiRegistrationInboxDraft() {
    ...
    renderDashboard();
    showAiRegistrationMessage("Lagret i CRM-innboks. Det er ikke opprettet kunde/henvendelse ennå.", "ok");
    ...
    el.aiRegistrationAttachments.innerHTML = aiRegistrationAttachments.map((item, index) => `
      <article>
        ...
        <div>
          ...
          <small>Lagres som privat CRM-vedlegg når du lagrer i innboks eller på kunde/henvendelse.</small>
        </div>
  ...
  function renderLeadAttachmentSection(entry, customer, leadTarget) {
    ...
    const actions = store.isConfigured && isAdmin() && (leadId || customer?.id) ? `
      <div class="section-actions">
        <button data-add-lead-attachment="${escapeHtml(leadTarget || key)}" type="button" title="Last opp bilde eller PDF på denne henvendelsen/saken.">Legg til bilde/vedlegg</button>
      </div>
    ` : "";
    return `
      <section class="detail-section">
  ...
  function handleCrmAttachmentClick(event) {
    ...
    if (addLead) {
      ...
      promptCrmAttachmentUpload({
        ...
        lead_id: leadId,
        source_kind: "lead",
        note: "Lastet opp fra henvendelse/tilbud.",
      });
      return true;
    }
      ...
      if (aiRegistrationAttachments.length) {
        ...
        aiRegistrationDraft = {
          ...
          analysis: {
            ...
            warnings: [{ code: "image_only", message: "Bilde er lagt ved. Fyll inn kundeinfo før du lagrer som kunde eller henvendelse.", severity: "info" }],
          },
        };
  ...
  async function saveAiRegistrationDraft() {
    ...
    if (values.action === "create_lead" && !store.isConfigured) {
      ...
      clearAiRegistration();
      setView("leads");
      setSyncStatus("Henvendelse opprettet lokalt fra hurtigregistrering.", "ok");
      return;
    }
    if (values.action === "create_lead" && store.saveLeadDraft) {
      ...
      if (sourceIntakeId) {
        ...
        await persistAiRegistrationAttachments({
          ...
          source_kind: "hurtigregistrering",
          note: "Vedlagt henvendelse fra hurtigregistrering.",
        });
      }
      ...
      clearAiRegistration();
      setView("leads");
      setSyncStatus("Henvendelse opprettet fra hurtigregistrering.", "ok");
      return;
    }
  ...
  function customerMoreActionsHtml(customer) {
    ...
    if (isAdmin() && !customer.is_inactive) {
      ...
      actions.push(`<button class="secondary" data-new-lead-existing-customer="${escapeHtml(key)}" type="button" title="Opprett ny henvendelse/tilbud på eksisterende kunde, f.eks. ekstra varmepumpe, befaring eller blåseisolering.">Ny oppfølging</button>`);
      ...
      if (isInsulationCustomer(customer)) actions.push(`<button class="secondary" data-new-lead-existing-customer="${escapeHtml(key)}" data-lead-kind="blaseisolering" type="button" title="Lag egen henvendelse/tilbud for blåseisolering på denne kunden.">Ny blåseisolering</button>`);
    }
    if (isAdmin()) {
  ...
  function customerRelationshipLabel(customer, installations = installationsForCustomer(customer)) {
    ...
    if (activeInstallations.length) {
      ...
      return "Kunde med anlegg";
    }
    if (customerOrders(customer).length) return "Kunde";
    if (isLeadCustomer(customer)) return "Kundeemne";
    return "Kundekort";
  }
  ...
  function installHelpText() {
    const help = [
      ...
      ["#aiRegistrationParseButton", "Lager forslag til kunde, henvendelsestype og historikk fra teksten. Ingenting lagres før du godkjenner."],
      ...
      ["#leadSearch", "Søk i henvendelser etter navn, telefon, sted, produkt, tags og notat."],
      ["#leadStatusFilter", "Filtrer henvendelser etter neste steg, for eksempel tilbud må sendes eller venter på svar."],
  ...
  async function createOrderFromLead(customerId, options = {}) {
    ...
    try {
      ...
      const productInterest = [lead?.product_interest, customer.brand, customer.model_or_note]
      ...
      await saveServiceEvent(customer, {
        ...
        note: [
          `${orderTypeLabel(type)} opprettet fra vunnet henvendelse.`,
          productInterest ? `Produkt/ønske: ${productInterest}.` : "",
          note || "",
      ...
      setView("orders");
      setSyncStatus("Jobb opprettet fra henvendelse.", "ok");
    } finally {
  ...
  function setNavigationActive(view) {
    ...
    el.mobileMoreButton?.classList.toggle("active", !isTechnicianUser() && ["technician", "insulation", "settings"].includes(view));
  ...
  function setView(view) {
    ...
    const titles = {
      ...
      leads: ["Innboks", "Nettside, e-post, henvendelser og oppfølging."],
    ...
    const items = [
      {
        ...
        help: "Nettside, e-postutkast og henvendelser som må avklares.",
      },
      {
        action: "followup",
        label: "Oppfølging",
  ...
  function renderIntakeInbox() {
    ...
    el.intakeInbox.innerHTML = `
      <div class="section-head compact">
        <div>
          <h3>${sourceTitle}</h3>
          <p title="Dette er lagrede utkast. Åpne og godkjenn før de blir kunde, henvendelse eller historikk.">Åpne og godkjenn.</p>
        </div>
  ...
  function leadCustomerFromRow(lead) {
    ...
    const name = lead?.company_name || [lead?.first_name, lead?.last_name].filter(Boolean).join(" ") || "Henvendelse uten navn";
    return {
      ...
      <div class="website-submission-list">
        ${visibleRows.map((row) => {
          ...
          const primaryAction = canCreateServiceOrder
            ...
            : `<button class="order-primary" data-create-website-lead="${escapeHtml(row.id)}" type="button" title="Oppretter eller kobler kundekort hvis nødvendig, og legger saken som salgsmulighet/henvendelse.">${escapeHtml(leadActionLabel)}</button>`;
          ...
          return `
            <article class="${isSelected ? "selected" : ""}" data-website-submission-card="${escapeHtml(row.id)}" tabindex="-1" title="${isSelected ? "Valgt fra søket. " : ""}Rå innsending beholdes uendret til den behandles server-side.">
              ...
              <div class="mini-action-row">
                ${primaryAction}
                <button class="secondary" data-website-submission-status="read" data-website-submission-id="${escapeHtml(row.id)}" type="button" title="Skjuler innsendingen uten å opprette kunde, henvendelse eller jobb.">Arkiver</button>
                <details class="inline-more-actions">
    ...
    if (websiteSubmissionIsInsulation(row)) {
      return {
        ...
        detail: duplicateCount
          ...
          : "Opprett eller koble kundekort, og legg saken som blåseisolering-henvendelse. Ikke lag jobb før kunden har sagt ja.",
      };
    }
    return {
      kind: duplicateCount ? "lead duplicate" : "lead",
      ...
      detail: duplicateCount
        ...
        : "Opprett eller koble kundekort, og legg henvendelsen som salgsmulighet.",
    };
  }
  ...
  async function deleteWebsiteSubmission(id) {
    ...
    const ok = await askForConfirmation({
      ...
      message: `Slette nettsideinnsendingen fra ${websiteSubmissionName(row)}? Eventuelt opprettet kundekort, henvendelse eller jobb slettes ikke.`,
      confirmLabel: "Slett",
      tone: "danger",
    });
    if (!ok) return;
  ...
  function leadCustomerDraft(entry) {
    ...
    const status = leadStatusForEntry(entry);
    const note = leadNoteForEntry(entry);
    const source = lead.source || customer.lead_source || "Henvendelse";
  ...
  async function createCustomerFromLead(entryKey) {
    ...
    renderAll();
    setView("leads");
    setSyncStatus("Kundekort opprettet fra henvendelse. Du kan nå sette status, opprette jobb eller booke avtale.", "ok");
  }
  ...
  function leadNextActionText(status, realCustomer, hasOrder = false) {
    ...
    if (status === "needs_offer") {
      return {
        ...
        body: "Bruk tilbudsfeltet under. Når kunden svarer ja, marker henvendelsen som vunnet og opprett jobb.",
      };
    }
  ...
  function leadNextActionHtml(entry, customer, realCustomer, status, leadTarget) {
    ...
    if (!realCustomer && entry?.lead) {
      actions.push(`<button class="order-primary" data-create-customer-from-lead="${escapeHtml(entryKey)}" type="button" title="Opprett kundekort fra henvendelsen. Bruk dette når det er en ny kunde og ingen tydelig dublett.">Opprett kundekort</button>`);
    }
    if (realCustomer && status === "followup") {
      actions.push(`<button class="order-primary" data-lead-set-status="needs_offer" data-lead-status-customer="${escapeHtml(leadTarget)}" type="button" title="Flytt henvendelsen til tilbudskøen når du har nok info til å lage tilbud.">Klar for tilbud</button>`);
    ...
    if (realCustomer && status === "won") {
      if (existingLeadOrder) {
        actions.push(`<button class="order-primary" data-open-order="${escapeHtml(existingLeadOrder.id)}" type="button" title="Denne henvendelsen har allerede jobb. Åpne jobben og book tid derfra.">Åpne jobb</button>`);
        ...
        actions.push(`<button class="order-primary" data-create-order-from-lead="${escapeHtml(leadTarget)}" type="button" title="Opprett jobb fra akkurat denne henvendelsen. Jobben kan planlegges etterpå.">Opprett jobb</button>`);
      ...
      actions.push(`<details class="inline-more-actions"><summary title="Snarveier og sjeldnere valg.">Mer</summary><div>
        ...
        <button class="secondary" data-new-lead-existing-customer="${escapeHtml(key)}" type="button" title="Lag en ny henvendelse/tilbud på samme kunde, for eksempel ekstra varmepumpe på hjemmeadresse eller hytte.">Ny oppfølging</button>
        ${entry?.lead && isAdmin() ? `<button class="secondary danger" data-delete-lead-entry="${escapeHtml(entryKey)}" type="button" title="Slett bare denne henvendelsen. Kundekort beholdes.">Slett henvendelse</button>` : ""}
      </div></details>`);
    }
    if (status === "lost") {
      actions.push(`<button class="secondary" data-lead-set-status="followup" data-lead-status-customer="${escapeHtml(leadTarget)}" type="button" title="Gjenåpne henvendelsen hvis kunden tar kontakt igjen.">Gjenåpne</button>`);
    }
    if (realCustomer && status !== "won") {
      actions.push(`<button class="secondary" data-new-lead-existing-customer="${escapeHtml(key)}" type="button" title="Lag en ny henvendelse/tilbud på samme kunde, for eksempel ekstra varmepumpe på hjemmeadresse eller hytte.">Ny oppfølging</button>`);
    }
    ...
    return `
      <section class="detail-section embedded-customer-card">
        <div class="section-title-row">
          <div>
            <h3>Kundekort</h3>
            <p title="Denne delen viser kundekort, anlegg, jobber, historikk og fakturaer direkte i henvendelsen.">Kundeinfo, anlegg og historikk.</p>
          </div>
          <div class="section-actions">
  ...
  function leadEmailToolsHtml(entry, customer, leadTarget, status) {
    ...
    if (status === "needs_offer") return `${templateSection}${offerSection}`;
  ...
  function renderLeadDetail() {
    ...
    el.leadDetail.innerHTML = `
      ...
      <section class="detail-section">
        ...
        <p class="context-hint">En henvendelse er ikke ferdig avklart. Når kunden sier ja, oppretter du jobb. Deretter planlegges jobben i kalenderen.</p>
        <details>
          <summary>Vis manuelle statusvalg</summary>
          ${canEditLead ? leadStatusControlHtml(customer, status, leadTarget) : `<p class="muted">Denne henvendelsen mangler kundekort. Opprett kundekort først når saken er reell og skal følges opp videre.</p>`}
    ...
    return `
      <section class="detail-section lead-offer-section">
        ...
        <div class="lead-offer-actions">
          ...
          <button class="order-primary" data-mark-offer-sent-manual="${escapeHtml(target)}" type="button" title="Bruk etter at tilbudet er sendt utenfor CRM. Lagrer historikk og flytter henvendelsen til venter svar.">Marker sendt manuelt</button>
        ...
        <div class="installation-list">
          ${visible.map((installation) => {
            ...
            if (isAdmin() && realInstallation) {
              ...
              secondaryInstallationActions.push(`<button class="secondary" data-new-lead-existing-customer="${escapeHtml(key)}" data-lead-kind="installasjon" data-lead-installation="${escapeHtml(installation.id || "")}" type="button" title="Lag en ny oppfølging/salgsmulighet som tydelig gjelder dette anlegget.">Ny henvendelse på anlegget</button>`);
              ...
              primaryInstallationActions.push(`<button data-new-installation-customer="${escapeHtml(key)}" data-installation-prefill="legacy" type="button" title="Åpner anleggsdialogen med gammel pumpe-/serviceinfo ferdig utfylt. Ingenting lagres før du trykker Lagre.">Gjør til anlegg</button>`);
            }
  ...
  function renderCustomerOpenLeads(customer) {
    ...
    ` : "";
    if (!entries.length) {
      return `<section class="detail-section"><div class="section-title-row"><h3>Åpne henvendelser / tilbud</h3>${actions}</div><div class="empty-state">Ingen åpne henvendelser eller tilbud på kunden.</div></section>`;
    }
    return `
      <section class="detail-section">
        <div class="section-title-row"><h3>Åpne henvendelser / tilbud</h3>${actions}</div>
        <div class="customer-order-list">
          ${entries.map((entry) => {
  ...
  el.customerDetail.addEventListener("click", (event) => {
    ...
    if (createOrderAndBook) {
      runBusyButton(createOrderAndBook, () => createOrderFromLead(createOrderAndBook.dataset.createOrderAndBookFromLead, { bookAfter: true }), "Klarte ikke opprette og booke jobb.");
      return;
    }
    const createOrder = event.target.closest("[data-create-order-from-lead]");
    if (createOrder) {
      runBusyButton(createOrder, () => createOrderFromLead(createOrder.dataset.createOrderFromLead), "Klarte ikke opprette jobb.");
      return;
    }
