(function () {
  ...
  function leadStatusProductLineFromSelection() {
    ...
    if (!selected || selected.id === "heatpump_custom") {
      ...
      if (!label) throw new Error("Søk eller velg vare/tillegg før du legger til.");
      return `- ${label}: ${formatJobPriceNumber(quantity)} stk - pris avtales/legges inn`;
    }
    const line = jobPriceLineForItem(selected, quantity);
  ...
  function setupLeadStatusProductFields(entry, customer) {
    ...
    if (el.leadStatusProduct) {
      el.leadStatusProduct.value = offerLines || initialLines || (prefill ? `- ${prefill}: pris avtales/legges inn` : "");
    }
    syncLeadStatusProductQuantity();
    renderLeadStatusProductTotal();
  }
  ...
  function planningDropPlacement(event, bookingId, day) {
    ...
    if (pointerPlacement) return pointerPlacement;
