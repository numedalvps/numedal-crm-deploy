(function () {
  ...
  function cleanLeadStatusProductPrefill(value) {
    const text = repairMojibakeForDisplay(String(value || "")).trim();
    if (!text) return "";
    const parts = text
      .split(/\s*(?:·|\||;|,|\n)\s*/)
      .map((part) => part.trim())
  ...
  function setupLeadStatusProductFields(entry, customer) {
    ...
    if (el.leadStatusProductSearch) el.leadStatusProductSearch.value = prefill;
    if (el.leadStatusProductPreset) {
      el.leadStatusProductPreset.innerHTML = offerLinePresetOptions("heatpump_custom", prefill);
    ...
    if (el.leadStatusProduct) {
      el.leadStatusProduct.value = initialLines || (prefill ? `- ${prefill}: pris avtales/legges inn` : "");
    }
    syncLeadStatusProductQuantity();
    renderLeadStatusProductTotal();
  }
