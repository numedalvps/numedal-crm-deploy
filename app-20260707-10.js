(function () {
  ...
  function websiteSubmissionLeadActionLabel(row) {
    return websiteSubmissionIsInsulation(row) ? "Lag kundekort + blåseisolering" : "Lag kundekort + tilbudssak";
  }
    ...
    try {
      ...
      if (duplicateRows.length) {
        const ok = await askForConfirmation({
          ...
          message: `Fant annen åpen nettsideinnsending som ligner: ${websiteSubmissionDuplicateSummary(row)}. Fortsett og lag kundekort + tilbud/oppfølging for valgt innsending?`,
  ...
  async function createServiceOrderFromWebsiteSubmission(id) {
    ...
    try {
      if (!websiteSubmissionIsService(row)) {
        throw new Error("Denne innsendingen ser ikke ut som service. Bruk Lag kundekort + tilbudssak hvis den skal følges opp som tilbud/salg.");
      }
      const values = websiteSubmissionLeadValues(row);
        ...
        <div class="lead-offer-builder">
          <div class="lead-offer-builder-head">
            <div>
              ...
              <small>Bygg opp produkter, tillegg og egne linjer før teksten sendes.</small>
            </div>
