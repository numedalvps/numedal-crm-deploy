(function () {
  ...
  function defaultOfferContextSubject(entry, templateId = "") {
    const brand = String(entry?.customer?.brand || entry?.lead?.preferred_brand || "").trim();
    const model = String(entry?.customer?.model_or_note || entry?.lead?.product_interest || "").trim();
    if (brand && model) {
      return normalizeMatch(model).startsWith(normalizeMatch(brand)) ? model : `${brand} ${model}`;
    }
