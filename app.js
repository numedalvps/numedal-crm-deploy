(function () {
  ...
  const storage = {
    ...
    installations: "numedalWebInstallations",
  ...
  const knownPlacePoints = new Map([
    ...
    ["blåberg", { lat: 59.77, lon: 9.25 }],
    ["blaberg", { lat: 59.77, lon: 9.25 }],
  ...
  const jobPriceItems = [
    ...
    { id: "ground_stand", articleNo: "9318", label: "Markkonsoll", aliases: "markstativ marskstativ bakkestativ stativ ute", unit: "stk", price: 1870, defaultQty: 1, eaccountingType: "goods", eaccountingUnit: "stk", eaccountingGroup: "CRM" },
  ...
  const offerLinePresets = [
    { id: "heatpump_custom", label: "Egen vare / skriv selv", unit: "stk", price: 0, defaultQty: 1 },
    ...heatPumpOfferItems,
    ...jobPriceItems,
  ];
  const rentalImages = [
  ...
  let globalSearchPointerHandled = false;
  ...
  let appHistoryGuardPushed = false;
  ...
  const el = {
    ...
    privateDataExportSettings: document.getElementById("privateDataExportSettings"),
    ...
    leadStatusProductSearch: document.getElementById("leadStatusProductSearch"),
    leadStatusProductPreset: document.getElementById("leadStatusProductPreset"),
    leadStatusProductQuantity: document.getElementById("leadStatusProductQuantity"),
    leadStatusAddProduct: document.getElementById("leadStatusAddProduct"),
    leadStatusStandardInstallationWrap: document.getElementById("leadStatusStandardInstallationWrap"),
    leadStatusIncludeStandardInstallation: document.getElementById("leadStatusIncludeStandardInstallation"),
    leadStatusProduct: document.getElementById("leadStatusProduct"),
    leadStatusProductTotal: document.getElementById("leadStatusProductTotal"),
    ...
    completionDialogMessage: document.getElementById("completionDialogMessage"),
    ...
    billingStandardInstallationWrap: document.getElementById("billingStandardInstallationWrap"),
    billingIncludeStandardInstallation: document.getElementById("billingIncludeStandardInstallation"),
    ...
    contactLogDialogMessage: document.getElementById("contactLogDialogMessage"),
  ...
  function applyModelDictionary(customer, extraText = "") {
    ...
    if (!customer.model_or_note) {
      ...
      else if (/\bhz25xke\b/.test(text)) customer.model_or_note = "HZ25XKE";
      else if (/\bhz25wke\b/.test(text)) customer.model_or_note = "HZ25WKE";
      else if (/\bhz25tke\b/.test(text)) customer.model_or_note = "HZ25TKE";
      else if (/\bhz25zke\b/.test(text)) customer.model_or_note = "HZ25ZKE";
      else if (/\bhz25(yke)?\b/.test(text)) customer.model_or_note = "HZ25";
      ...
      else if (/\bhara\b/.test(text)) customer.model_or_note = "Hara";
      else if (/\bgussuri\b/.test(text)) customer.model_or_note = "Gussuri";
      else if (/\bpolar\b/.test(text)) customer.model_or_note = "Polar";
      else if (/\bdaisekai\b/.test(text)) customer.model_or_note = "Daisekai";
      else if (/\bnarvik\s*25\b/.test(text)) customer.model_or_note = "Narvik 25";
      else if (/\bnarvik\s*35\b/.test(text)) customer.model_or_note = "Narvik 35";
      else if (/\btrysil\b/.test(text)) customer.model_or_note = "Trysil";
      else if (/\barctic\s*12\b/.test(text)) customer.model_or_note = "Arctic 12";
      else if (/\bnorgespumpa\s*6[,.]4\b/.test(text)) customer.model_or_note = "Norgespumpa 6.4";
      else if (/\bnorgespumpa\s*5[,.]9\b/.test(text)) customer.model_or_note = "Norgespumpa 5.9";
      else if (/\bnorgespumpa\s*5[,.]7\b/.test(text)) customer.model_or_note = "Norgespumpa 5.7";
    }
  }
  ...
  function customerIsMarkedCustomer(customer) {
    ...
    if (activeRealInstallationsForCustomer(customer).length) return true;
  ...
  function markCustomerAsCustomer(customer) {
    if (!customer) return;
    customer.tags = nextTagsWithCustomerMarker(customer.tags, true);
  }
  ...
  function activityMatchesLeadEntry(activity, entry) {
    ...
    return (
      (leadIds.size && leadIds.has(String(activity.lead_id || activity.leadId || metadata.lead_id || metadata.leadId || "")))
      || (customerKeys.size && customerKeys.has(String(activity.customer_id || activity.customerId || metadata.customer_id || metadata.customerId || metadata.customer_key || metadata.customerKey || metadata.customer_lime_id || "")))
    );
  }

  function offerActivitiesForLeadEntry(entry) {
    return (activities || [])
      .filter((activity) => activityMatchesLeadEntry(activity, entry))
      .filter((activity) => /offer|tilbud/i.test(`${activity.activity_type || ""} ${activity.summary || ""} ${activity.body || ""}`))
  ...
  function leadStatusDetailLines(status, details = {}) {
    ...
    const priceBasis = String(details.priceBasis || "").trim();
    const note = String(details.note || "").trim();
    if (product) lines.push(`Varmepumpe/produkt: ${product}`);
    if (priceBasis) lines.push(`Avtalt grunnlag:\n${priceBasis}`);
  ...
  function leadStatusPatch(status, details = {}) {
    const patch = { status };
    const product = cleanLeadStatusProductPrefill(details.productInterest || details.product || "");
  ...
  async function setLeadRecordStatus(entryKey, status, details = {}) {
    ...
    if (!entry?.lead?.id) throw new Error("Fant ikke saken.");
  ...
  async function deleteLeadEntry(entryKey) {
    const entry = leadEntryForTarget(entryKey);
    if (!entry?.lead?.id) throw new Error("Fant ikke saken som skulle slettes.");
  ...
  function extractAiAddress(text) {
    ...
    const withZip = String(text || "").match(/([A-ZÆØÅ][A-Za-zÆØÅæøå0-9 .,'-]{3,80}?\s+\d+[A-Za-z]?)\s*,?\s+(\d{4})\s*,?\s+([A-ZÆØÅ][A-Za-zÆØÅæøå .'-]+)/);
    if (withZip) {
      return {
  ...
  function addressPartsFromFreeText(value, fallback = {}) {
    const raw = String(value || "").trim();
    const zip = String(fallback.zip || fallback.postal_code || "").trim();
    const fallbackCity = String(fallback.city || "").trim();
    if (!raw) {
      return {
        street: "",
        zip,
        city: fallbackCity || postalCityByZip[zip] || "",
      };
    }

    const parsed = extractAiAddress(raw);
    let street = cleanAiLine(parsed.street || "");
    const parsedZip = parsed.zip || zip;
    const city = cleanAiLine(parsed.city || fallbackCity || postalCityByZip[parsedZip] || "");
    const normalizedRaw = normalizeMatch(raw);
    const messageLike = raw.length > 120
      || /\?/.test(raw)
      || /\b(hei|onsker|nsker|ønsker|befaring|varmepumpe|tilbud|service|kan du|kan dere)\b/.test(normalizedRaw);

    if (!street && !messageLike) street = cleanAiLine(raw);
    if (street.length > 90 || /\b(hei|onsker|nsker|ønsker|befaring|varmepumpe|tilbud|service|kan du|kan dere)\b/.test(normalizeMatch(street))) {
      street = "";
    }
    return { street, zip: parsedZip, city };
  }
  ...
  function aiRegistrationSourceKind(draftOrValues = aiRegistrationDraft) {
    const raw = String(draftOrValues?.raw || "");
    const analysisKind = String(draftOrValues?.analysis?.source?.kind || "").toLowerCase();
    const text = normalizeMatch([
      analysisKind,
      raw,
      draftOrValues?.note,
    ].filter(Boolean).join(" "));
    if (analysisKind === "email" || /\b(fra|from|sendt|sent|til|to|emne|subject)\b/.test(text) && /@/.test(raw)) return "email";
    if (analysisKind === "website" || /\b(nettside|website|kontaktskjema|contact form|webskjema)\b/.test(text)) return "website";
    return "manual";
  }

  function aiRegistrationSourceLabel(draftOrValues = aiRegistrationDraft) {
    const kind = aiRegistrationSourceKind(draftOrValues);
    if (kind === "email") return "E-post hurtigregistrering";
    if (kind === "website") return "Nettside hurtigregistrering";
    return "Hurtigregistrering";
  }

  function aiRegistrationNeedsOffer(raw, analysis) {
    const category = analysis?.intent?.category || "";
    if (category === "quote_request") return true;
    return /skal ha tilbud|trenger tilbud|ønsker tilbud|onsker tilbud|kan du sende tilbud|send tilbud|lage tilbud|pris på|pris pa|hva koster/i.test(String(raw || ""));
  }

  function aiTagsForDraft(type, raw = "", analysis = null) {
    const tags = splitTags(aiTagsForType(type));
    if (type === "lead" && aiRegistrationNeedsOffer(raw, analysis)) {
      return nextTagsWithLeadStatus(tags, "needs_offer");
    }
  ...
  function aiRegistrationSelectedCustomerNoticeHtml() {
    ...
    if (!selectedCustomer) {
      return `
        <div class="ai-selected-customer-notice empty">
    ...
    return `
      <div class="ai-selected-customer-notice">
  ...
  async function saveAiRegistrationDraft() {
    ...
    const shouldSyncLeadFromAi = values.action !== "history_only"
      && (["lead", "befaring", "installasjon", "blaseisolering"].includes(values.type) || isLeadCustomer(saved));
    const syncedLead = shouldSyncLeadFromAi
    ...
    const shouldOpenLead = shouldSyncLeadFromAi || Boolean(syncedLead);
    if (shouldOpenLead) {
  ...
  async function copyRouteBulkSms() {
    ...
    await copyTextWithStatus(text, `SMS-tekster kopiert for ${routePlannerRows.length} kunder.`, "SMS-tekstene");
  }
          ...
          ${dayPlan.overflow.map((row) => `
            <article class="route-overflow-card" data-route-overflow-customer="${escapeHtml(customerKey(row.customer))}" tabindex="0" title="Trykk for kundeinfo. Hold over for handlinger.">
              ...
              <div class="route-overflow-actions">
                ${row.customer.phone ? `${smsActionButton(row.customer.phone)}${copyPhoneButton(row.customer.phone, "Kopier nr")}` : ""}
  ...
  function customerPrimaryActionsHtml(customer) {
    ...
    if (isAdmin() && !customer.is_inactive) {
      ...
      actions.push(`<button class="book-primary" data-book-customer="${escapeHtml(key)}" type="button" title="Book tid i kalenderen. Dette lager jobbutkast hvis det mangler.">Book tid</button>`);
      actions.push(`<button class="reminder-primary" data-new-reminder-customer="${escapeHtml(key)}" type="button" title="Lag rask påminnelse om å ringe, sende tilbud eller følge opp kunden.">Påminnelse</button>`);
    }
  ...
  function customerMoreActionsHtml(customer) {
    ...
    if (isAdmin() && !customer.is_inactive) {
      actions.push(`<button class="secondary" data-new-order-customer="${escapeHtml(key)}" type="button" title="Lag jobb uten å velge dato nå. Bruk Book tid hvis tidspunktet skal settes med en gang.">Ny jobb uten dato</button>`);
  ...
  function customerServiceSummary(customer, installations = installationsForCustomer(customer)) {
    ...
    if (cleanLegacyCustomerProductText(customer) || customer?.first_install_date || customer?.next_service_due) return "Merke/modell på kundekort";
  ...
  function syncStatusLooksLikeLoading() {
    return /laster|oppdaterer|innlogging bekreftet/i.test(el.syncStatus?.textContent || "");
  }
  ...
  async function supabaseLoad() {
    ...
    if (firstLoad) {
      renderApp();
      setSyncStatus("Innlogging bekreftet. Laster kundedata, jobber og historikk...", "");
    } else {
      setSyncStatus("Oppdaterer kundedata, jobber og historikk...", "");
    }
  ...
  function orderForLeadEntry(entry) {
    const direct = orderForLeadId(entry?.lead?.id || "");
    if (direct) return direct;
    const customer = entry?.customer ? (findCustomer(leadEntryCustomerKey(entry)) || entry.customer) : null;
    if (!customer) return null;
    const candidates = customerOrders(customer).filter((order) => (
  ...
  function bookingWorkflowState(row, order = null) {
    ...
    if (billingStatus === "draft" || bookingNeedsInvoiceFinalAction(row)) {
      steps[3] = workflowStep("Utkast", "current", "Fakturautkast er laget/eksportert, men faktura er ikke markert sendt.");
      return {
        kind: "billing",
  ...
  function orderWorkflowState(order, linkedRows = []) {
    ...
    const steps = [
      ...
      workflowStep("Faktura", billing === "ready" ? "warn" : billing === "draft" ? "current" : billing === "sent" || billing === "paid" ? "done" : "pending", "Faktura/betaling avklares etter utført jobb."),
  ...
  function bookingWorkflowButtons(row, options = {}) {
    ...
    return [
      ...
      options.includeEdit ? `<button class="secondary" data-edit-booking="${escapeHtml(row.id)}" type="button">Endre tid</button>` : "",
    ].filter(Boolean).join("");
  }
  ...
  async function saveOrderRecord(id, order, options = {}) {
    ...
    const hasLocationId = Object.prototype.hasOwnProperty.call(order, "locationId")
      || Object.prototype.hasOwnProperty.call(order, "location_id");
    const hasLeadId = Object.prototype.hasOwnProperty.call(order, "leadId")
      || Object.prototype.hasOwnProperty.call(order, "lead_id");
    const leadId = hasLeadId ? (order.leadId ?? order.lead_id ?? "") : (mirrorJob?.lead_id || "");
    ...
    if (store.isConfigured && store.saveOrder && !options.localOnly) {
      ...
      const savedWithLinks = {
        ...saved,
        leadId: saved.leadId || saved.lead_id || record.leadId || record.lead_id || "",
        lead_id: saved.lead_id || saved.leadId || record.lead_id || record.leadId || "",
      };
      orders[savedWithLinks.id] = savedWithLinks;
      upsertLocalJobMirrorForOrder(savedWithLinks);
      if (nextId !== savedWithLinks.id) delete orders[nextId];
      return savedWithLinks;
    }
  ...
  async function markOrderInvoicedFromDetail(orderId) {
    const order = findOrder(orderId);
    if (!order) throw new Error("Fant ikke jobben.");
    const customer = findCustomer(orderCustomerId(order));
    if (!customer) throw new Error("Fant ikke kundekortet til jobben.");
    const invoiceDate = isoDate(new Date());
    const saved = await saveOrderRecord(order.id, {
      ...order,
      status: order.status === "cancelled" ? "cancelled" : "completed",
      billingStatus: "sent",
      completedAt: order.completedAt || order.completed_at || invoiceDate,
      invoicedAt: invoiceDate,
    }, { quiet: true });
    const linkedJob = jobForOrder(saved);
    const priceBasis = extractJobPriceBasis(saved.note || order.note);
    const installationText = orderInstallationText(saved, customer, linkedJob);
    const body = [
      `${orderTypeLabel(saved.type)} markert fakturert ${formatDate(invoiceDate)}.`,
      saved.title ? `Jobb: ${saved.title}.` : "",
      installationText ? `Anlegg/adresse: ${installationText}.` : "",
      priceBasis ? `Fakturagrunnlag/tillegg:\n${priceBasis}` : "",
    ].filter(Boolean).join("\n");
    const warnings = [];
    try {
      await saveServiceEvent(customer, {
        event_date: invoiceDate,
        event_type: "Fakturert",
        note: body,
      });
    } catch (error) {
      warnings.push(error.message || "historikk");
    }
    try {
      await saveActivityRecord({
        customer_id: customer.id || customerKey(customer),
        job_id: linkedJob?.id || saved.jobId || saved.job_id || "",
        activity_type: "job_invoiced",
        summary: "Jobb markert fakturert",
        body,
        metadata: {
          order_id: saved.id || order.id,
          billing_status: "sent",
          source: "order_detail",
        },
      });
    } catch (error) {
      warnings.push(error.message || "aktivitet");
    }
    renderAll();
    if (warnings.length) {
      setSyncStatus(`Jobb markert fakturert, men historikk/aktivitet ble ikke fullstendig lagret: ${warnings.join(" / ")}`, "error");
    } else {
      setSyncStatus("Jobb markert fakturert og lagret i kundehistorikk.", "ok");
    }
  }

  function leadOrderContextNote(note) {
    const raw = String(note || "").trim();
    if (!raw) return "";
    if (offerLinesFromText(raw) || /\bCRM-ref:\s*NVS-TILBUD-/i.test(raw) || /Prisliste\s*\/\s*prisgrunnlag/i.test(raw)) return "";
    const referenceSplit = splitEmailReferenceFooter(raw);
    return cleanBookingNote(stripOfferDocumentBlock(stripOfferLinesBlock(stripOfferContextBlock(referenceSplit.body)))).trim();
  }
  ...
  async function createOrderFromLead(customerId, options = {}) {
    ...
    if (!customer) throw new Error("Fant ikke sak/kunde.");
    ...
    try {
      ...
      const explicitPriceBasis = String(options.priceBasis || "").trim();
      const sourceDetail = String(lead?.source_detail || customer.latest_deal_name || "").trim();
      const contextNote = leadOrderContextNote(note);
      ...
      const type = /\b(blaseisolering|isobygg|isolering|supafil)\b/.test(leadText)
        ? "blaseisolering"
        : leadOrderLooksLikeInstallationSale(productInterest, explicitPriceBasis, accessories)
          ? "installasjon"
          : /\b(service|reparasjon|feil|drypp|stoy|støy)\b/.test(leadText)
          ? "service"
          : "installasjon";
      const priceBasis = explicitPriceBasis || (type === "installasjon"
        ? priceBasisFromSalesDetails(productInterest, accessories)
        : priceBasisFromSalesDetails("", accessories));
      const existingLeadOrder = orderForLeadEntry(entry) || orderForLeadId(leadId);
      ...
      const orderNote = noteWithJobPriceBasis([
        ...
        contextNote || `Kunden har takket ja. Status: ${leadStatusLabel(leadStatusForCustomer(customer))}.`,
      ...
      openOrderById(order.id, { filter: "unscheduled" });
  ...
  function priceBasisLinesFromText(text, options = {}) {
    ...
    for (const term of terms) {
      ...
      if (!item || item.id === "heatpump_custom") {
        lines = appendUniquePriceBasisLine(lines, customPriceLineFromText(term, 1, "stk"));
        continue;
      }
  ...
  function invoiceDraftMarkerRegex() {
    return /\[Fakturautkast[^\]]*\]/i;
  }

  function invoiceDraftMarkersRegex() {
  ...
  function cleanBookingNote(note) {
    return String(note || "")
      .replace(invoiceDraftMarkersRegex(), "")
  ...
  function bookingLinkedBillingStatus(row) {
    const order = row ? linkedOrderForBooking(row.id) || findOrder(row.booking.orderId) : null;
    if (!order) return "";
    return orderBillingStatusFromJob(jobForOrder(order)) || order.billingStatus || order.billing_status || "";
  }

  function bookingHasInvoiceDraft(row) {
    return Boolean(invoiceDraftMarkerRegex().test(row?.booking?.note || "") || bookingLinkedBillingStatus(row) === "draft");
  }
  ...
  function bookingNeedsInvoice(row) {
    ...
    if (bookingHasInvoiceDraft(row)) return false;
    return row.booking.status === "done" || doneJobs.has(row.id);
  }

  function bookingNeedsInvoiceFinalAction(row) {
    if (!row || bookingPaymentDone(row)) return false;
    if (row.customer?.pays_cash) return false;
    if (!billableJobType(bookingDisplayType(row))) return false;
    if (!bookingHasInvoiceDraft(row)) return false;
  ...
  function bookingNeedsPaymentAction(row) {
    return bookingNeedsInvoice(row) || bookingNeedsInvoiceFinalAction(row) || bookingNeedsCashPayment(row);
  }
  ...
  function appHistoryUrl() {
    return `${window.location.origin}${window.location.pathname}`;
  }

  function appHistoryState(view = currentView) {
    return { numedalCrm: true, view: view || defaultViewForCurrentUser() };
  }

  function isAppHistoryState(state) {
    return Boolean(state && state.numedalCrm === true);
  }

  function ensureAppHistoryGuard() {
    if (!currentUser || passwordRecoveryActive || !window.history?.replaceState || !window.history?.pushState) return;
    const state = appHistoryState();
    const url = appHistoryUrl();
    if (!isAppHistoryState(window.history.state)) {
      window.history.replaceState(state, "", url);
    }
    if (!appHistoryGuardPushed) {
      window.history.pushState(state, "", url);
      appHistoryGuardPushed = true;
    }
  }

  function updateAppHistory(previousView, options = {}) {
    if (options.fromHistory || !currentUser || passwordRecoveryActive || !window.history?.replaceState) return;
    ensureAppHistoryGuard();
    const state = appHistoryState();
    const url = appHistoryUrl();
    if (previousView && previousView !== currentView && window.history?.pushState) {
      window.history.pushState(state, "", url);
  ...
  function handleNewAction(action) {
    ...
    if (action === "job") {
      setView("orders");
      setSyncStatus("Velg en kunde eller åpne et kundekort for å lage ny jobb. Book tid brukes når du vil velge dato og tid med en gang.", "ok");
    }
  }
  ...
  function renderStartWorklist(counts = {}) {
    ...
    el.startWorklist.innerHTML = items.map((item) => `
      <button data-start-action="${escapeHtml(item.action)}" type="button" title="${escapeHtml(item.help)}">
  ...
  function globalSearchRows(query = globalSearchQuery) {
    ...
    const rank = { Kunde: 0, Anlegg: 1, Lead: 2, Nettside: 3, Jobb: 4, Avtale: 5, Faktura: 6, Påminnelse: 7 };
    return results
  ...
  function openGlobalSearchResult(id) {
    ...
    if (row.kind === "Påminnelse") {
      ...
      setView("dashboard");
      setTimeout(() => {
        const card = row.reminderId
          ? el.reminderQueue?.querySelector(`[data-open-reminder="${CSS.escape(row.reminderId)}"]`)
          : null;
        (card || el.reminderQueue?.closest(".panel"))?.scrollIntoView({ behavior: "smooth", block: "center" });
  ...
  function renderCustomers() {
    ...
    for (const customer of list.slice(0, 250)) {
      ...
      button.innerHTML = `<span class="dot ${statusKind(customer)}"></span><strong>${customerStarHtml(customer)}${customerCashBadgeHtml(customer)}${escapeHtml(cleanDisplayName(customer))}</strong><small>${leadBadgeHtml(customer)}${escapeHtml(customer.location_tag || customer.visit_city || "Ukjent sted")} · ${escapeHtml(customerProductDisplayText(customer, "Produkt ikke registrert"))} · ${formatDate(nextServiceDueForCustomer(customer))}</small>`;
  ...
  function leadCardPreview(entry) {
    const text = repairTextEncoding(leadCardSummary(entry)).replace(/\s+/g, " ").trim();
  ...
  function filteredLeads() {
    ...
    return allLeadEntries()
      .filter((entry) => {
        const status = leadStatusForEntry(entry);
        if (hasSearch) return !leadEntryLooksLikeSpam(entry);
  ...
  function renderLeads() {
    ...
    for (const entry of list.slice(0, 250)) {
      ...
      const key = leadEntryKey(entry);
      const status = leadStatusForEntry(entry);
      const realCustomer = Boolean(findCustomer(leadEntryCustomerKey(entry)));
      const next = leadNextActionText(status, realCustomer, Boolean(orderForLeadEntry(entry)));
    ...
    const openHtml = rows.length ? `
      <div class="section-head compact">
        <div>
          ...
          <p title="Dette er rå skjema fra nettsiden. Første trygge steg er kundekort, deretter tilbud, service eller jobb.">Første steg er kundekort. Velg tilbud eller service etterpå.</p>
        </div>
        <strong>${rows.length.toLocaleString("nb-NO")}</strong>
      </div>
      <div class="website-submission-list">
        ${visibleRows.map((row) => {
          ...
          const primaryAction = canCreateServiceOrder
            ? `<button class="order-primary" data-create-website-service-order="${escapeHtml(row.id)}" type="button" title="Opprett eller koble kundekort, og lag en servicejobb som kan bookes etter kontakt med kunden.">Lag kundekort + service</button>`
  ...
  function websiteSubmissionCustomer(row) {
    ...
    return {
      ...merged,
      name: firstFilled(direct.name, row?.customer_name, row?.name, nested.name, contact.name, formCustomer.name, formContact.name, payload.customer_name, payload.name, payload.navn, payload.fullName, form.customer_name, form.name, form.navn, form.fullName),
      phone: firstFilled(direct.phone, row?.phone, row?.normalized_phone, nested.phone, contact.phone, formCustomer.phone, formContact.phone, payload.phone, form.phone),
      email: firstFilled(direct.email, row?.email, row?.normalized_email, nested.email, contact.email, formCustomer.email, formContact.email, payload.email, form.email),
      address: firstFilled(direct.address, direct.street, row?.address, row?.street, nested.address, nested.street, contact.address, contact.street, formCustomer.address, formCustomer.street, formContact.address, formContact.street, payload.address, payload.adresse, form.address, form.adresse),
      postal_code: postalCode,
      zip: postalCode,
      city: firstFilled(direct.city, row?.city, nested.city, contact.city, formCustomer.city, formContact.city, payload.city, form.city, postalCityByZip[postalCode]),
    };
  }
  ...
  function websiteSubmissionRequest(row) {
    ...
    return {
      ...merged,
      request_type: firstFilled(direct.request_type, row?.request_type, nested.request_type, formRequest.request_type, payload.request_type, form.request_type),
      service_reason: firstFilled(direct.service_reason, row?.service_reason, nested.service_reason, formRequest.service_reason, payload.service_reason, form.service_reason),
      preferred_contact_method: firstFilled(direct.preferred_contact_method, row?.preferred_contact_method, nested.preferred_contact_method, formRequest.preferred_contact_method, payload.preferred_contact_method, form.preferred_contact_method),
      address: firstFilled(direct.address, row?.address, nested.address, formRequest.address, payload.address, payload.adresse, form.address, form.adresse),
      message: firstFilled(direct.message, row?.message, nested.message, formRequest.message, payload.message, payload.melding, form.message, form.melding, form.description, form.comment, form.kommentar),
    };
  }
  ...
  function websiteSubmissionSupport(row) {
    ...
    return {
      ...
      ...nested,
      ...direct,
      symptom_slug: firstFilled(direct.symptom_slug, row?.symptom_slug, nested.symptom_slug, formSupport.symptom_slug, payload.symptom_slug, form.symptom_slug),
      severity: firstFilled(direct.severity, row?.severity, nested.severity, formSupport.severity, payload.severity, form.severity),
      preferred_contact_method: firstFilled(direct.preferred_contact_method, row?.preferred_contact_method, nested.preferred_contact_method, formSupport.preferred_contact_method, payload.preferred_contact_method, form.preferred_contact_method),
      message: firstFilled(direct.message, row?.message, nested.message, formSupport.message, payload.message, payload.melding, form.message, form.melding, form.description, form.comment, form.kommentar),
  ...
  function websiteSubmissionSourcePage(row) {
    ...
    return firstFilled(attribution.page_url, attribution.landing_page, payload.source_page, payload.page_url, payload.landing_page, form.source_page, form.page_url, form.landing_page, row?.landing_page);
  }
  ...
  function websiteSubmissionDetailsHtml(row) {
    ...
    const messageHtml = message ? `
      <div class="website-submission-message">
        <strong>Kunden skrev fra nettsiden</strong>
        <p>${escapeHtml(message.slice(0, 3000))}${message.length > 3000 ? "\n..." : ""}</p>
      </div>
    ` : "";
  ...
  function websiteSubmissionLeadActionLabel(row) {
    return websiteSubmissionIsInsulation(row) ? "Lag kundekort + blåseisolering" : "Lag kundekort + tilbud";
  }
  ...
  async function createLeadFromWebsiteSubmission(id) {
    if (!store.saveCustomer || !store.saveLeadDraft || !store.updateWebsiteSubmission) {
        throw new Error("Nettsideinnsendinger krever Supabase og oppdatert adapter med kundekort og saker.");
    }
    ...
    try {
      ...
      if (duplicateRows.length) {
        const ok = await askForConfirmation({
          ...
          message: `Fant annen åpen nettsideinnsending som ligner: ${websiteSubmissionDuplicateSummary(row)}. Fortsett og lag kundekort + tilbud for valgt innsending?`,
  ...
  async function linkLeadToExistingCustomer(entryKey, customerId) {
    if (store.isConfigured && !store.updateLead) throw new Error("Kobling av sak krever oppdatert Supabase-adapter.");
  ...
  async function createCustomerFromLead(entryKey) {
    if (store.isConfigured && (!store.saveCustomer || !store.updateLead)) throw new Error("Opprett kundekort fra sak krever oppdatert Supabase-adapter.");
    if (!store.isConfigured) requireLocalDemoStorage();
    const entry = allLeadEntries().find((item) => leadEntryKey(item) === entryKey);
    if (!entry?.lead) throw new Error("Fant ikke saken.");
  ...
  function leadNextActionText(status, realCustomer, hasOrder = false) {
    ...
    if (status === "won") {
      return hasOrder
        ? {
          heading: "Åpne jobb",
          body: "Saken er vunnet og har jobb. Gå videre til booking, utføring og fakturering.",
        }
        : {
          heading: "Lag jobb",
  ...
  function leadReadinessItems(entry, customer, realCustomer, status, hasOrder = false) {
    ...
    if (status === "won") {
      items.push({ label: "Jobb", ok: hasOrder, hint: "Opprett jobb før booking og faktura." });
    }
    return items;
  }

  function leadReadinessHtml(entry, customer, realCustomer, status) {
    if (status === "lost") return "";
    const hasOrder = Boolean(orderForLeadEntry(entry));
  ...
  function leadNextActionHtml(entry, customer, realCustomer, status, leadTarget) {
    ...
    if (realCustomer && status === "needs_offer") {
      ...
      secondaryActions.push(`<button class="secondary" data-lead-set-status="offer_sent" data-lead-status-customer="${escapeHtml(leadTarget)}" type="button" title="Brukes bare når tilbudet faktisk er sendt utenfor CRM eller bekreftet sendt.">Tilbud er sendt</button>`);
    }
    ...
    return `
      <section class="lead-next-action ${escapeHtml(status)}">
        <div>
          <span>${escapeHtml(leadStatusLabel(status))}</span>
  ...
  function leadStatusOfferLinesPrefill(entry) {
    if (!entry) return "";
    const sources = [
      ...offerActivitiesForLeadEntry(entry).map((activity) => [
        activity.body,
        activity.summary,
  ...
  function openLeadStatusDialog(target, status, options = {}) {
    ...
    setupLeadStatusProductFields(entry, customer);
  ...
  function leadStatusDialogValues() {
    ...
    return {
      ...state,
      customer,
      productInterest: includeOrderDetails ? (leadStatusProductSummaryFromLines(el.leadStatusProduct?.value || "") || leadStatusProductPrefill(leadEntryForTarget(state.target || ""), customer)) : "",
        ...
        <details>
          ...
          <div class="lead-advanced-grid">
            ...
            <div class="lead-status-actions">
              ...
              ${realCustomer && !leadForCustomer(customer) ? `<button class="secondary" data-inactivate-lead="${escapeHtml(key)}" type="button" title="Skjul denne gamle leadmarkeringen fra aktiv liste. Kundekortet beholdes.">Skjul sak</button>` : ""}` : ""}
            </div>
          </div>
        </details>
      </section>
  ...
  function renderLeadDetail() {
    ...
    el.leadDetail.innerHTML = `
      ...
      ${leadWorkflowHtml(status, Boolean(orderForLeadEntry(entry)))}
      ${leadNextActionHtml(entry, customer, realCustomer, status, leadTarget)}
      ${leadReadinessHtml(entry, customer, realCustomer, status)}
  ...
  function customerNextAction(customer, options = {}) {
    ...
    if (counts.openLeads) {
      return { label: "Følg opp sak", detail: `${counts.openLeads.toLocaleString("nb-NO")} åpen oppfølging/tilbud.`, tone: "attention" };
    }
    if (counts.billingReady) {
      return { label: "Fakturer jobb", detail: `${counts.billingReady.toLocaleString("nb-NO")} jobb ligger klar.`, tone: "attention" };
    }
    if (counts.reminders) {
      const reminder = counts.nextReminder;
      const urgency = reminder ? reminderUrgency(reminder) : { className: "neutral" };
      ...
      const detail = reminder
        ? `${reminderDueLabel(reminder)} · ${shortEventNote(reminder.body || summary) || "Åpen påminnelse"}`
        : `${counts.reminders.toLocaleString("nb-NO")} åpen påminnelse.`;
      return { label: summary, detail, tone };
    }
    if (counts.activeOrders) {
      return { label: "Følg jobb", detail: `${counts.activeOrders.toLocaleString("nb-NO")} aktiv jobb/avtale.`, tone: "neutral" };
    }
    if (counts.serviceAttention) {
      return { label: "Vurder service", detail: "Servicefrist er aktuell eller forfalt.", tone: "attention" };
    }
    if (counts.missingContact) {
      return { label: "Sjekk kontaktinfo", detail: "Telefon, e-post eller adresse mangler.", tone: "neutral" };
    }
  ...
  function renderCustomerWorkSummary(customer, options = {}) {
    ...
    const visibleJobCount = counts.billingReady || counts.activeOrders || 0;
    const jobDetail = counts.billingReady
      ...
      : counts.activeOrders
        ...
        : counts.orders
          ? "Kun historikk"
          : "Ingen jobber";
    const installationDetail = counts.serviceAttention
      ? "Service bør sjekkes"
    ...
    return `
      <section class="customer-work-summary ${options.compact ? "compact" : ""}" aria-label="Arbeidsoversikt for kunde">
        <article class="${escapeHtml(next.tone)}">
        ...
        <article>
          <span>Jobber</span>
          <strong>${visibleJobCount.toLocaleString("nb-NO")}</strong>
  ...
  function renderCustomerContactPulse(customer) {
    ...
    const contactRows = activitiesForCustomer(customer)
      ...
      .slice(0, 3);
    const adminActions = isAdmin() && !customer?.is_inactive ? `
      <div class="customer-pulse-actions">
        <button class="contact-primary" data-contact-log-customer="${escapeHtml(key)}" type="button">Logg kontakt</button>
        <button class="reminder-primary" data-new-reminder-customer="${escapeHtml(key)}" type="button">Påminnelse</button>
      </div>
    ` : "";
    const contactList = contactRows.length
      ...
      : `<li class="muted"><strong>Ingen kontaktlogg ennå</strong><span>Bruk Logg kontakt etter telefon, SMS eller e-post.</span></li>`;
    const reminderList = reminderRows.length
      ...
      : `<li class="muted"><strong>Ingen åpne påminnelser</strong><span>Kunden har ingen aktiv oppfølging akkurat nå.</span></li>`;
    return `
      <section class="customer-contact-pulse">
        <div class="customer-pulse-head">
          <div>
            <h3>Siste kontakt / neste steg</h3>
            <p>Rask oversikt over hva som nettopp er sagt, og hva som må følges opp.</p>
          </div>
          ${adminActions}
        </div>
        <div class="customer-pulse-grid">
          <article>
            <span>Siste kontakt</span>
            <ul>${contactList}</ul>
          </article>
          <article>
            <span>Åpen oppfølging</span>
            <ul>${reminderList}</ul>
          </article>
        </div>
      </section>
  ...
  function offerActivityRowsForCustomer(customer) {
    return activitiesForCustomer(customer)
      .filter((activity) => ["email_offer_sent", "email_offer_draft"].includes(String(activity.activity_type || "").toLowerCase()))
      .slice(0, 4);
  }
  ...
  function offerActivityDetails(activity, customer = null) {
    ...
    const total = asOptions ? 0 : metadataTotal > 0 ? metadataTotal : jobPriceBasisTotal(lines || text);
    const product = metadata.offer_product
    ...
    return {
      sent: String(activity.activity_type || "").toLowerCase() === "email_offer_sent",
      product,
      lines,
      total,
      reference,
      ...
      <section class="detail-section customer-offer-summary">
        ...
        <div class="customer-offer-list">
          ${rows.map((activity) => {
            ...
            const status = details.sent ? "Sendt" : "Utkast";
            const meta = [
              details.to ? `Til ${details.to}` : "",
            ...
            return `
              <article class="${details.sent ? "sent" : "draft"}">
                <div>
                  <time>${formatDate(activity.occurred_at || activity.created_at)}</time>
  ...
  function selectedOfferAttachments(includePriceList) {
    return includePriceList ? [offerDocumentAttachment(priceListDocument)] : [];
  }
      ...
      <section class="detail-section lead-offer-section">
        ...
        <div class="lead-offer-documents">
          <label class="check-row">
            ...
            <span>
              <strong>Legg ved prisliste-PDF</strong>
              <small>Send fra CRM legger PDF-en ved e-posten. Kopi/utkast har lenke i teksten og kan lastes ned her.</small>
            </span>
          </label>
        ...
        <div class="lead-offer-actions">
          <div class="lead-offer-primary-actions">
            ...
            <button class="secondary" data-copy-offer-draft="${escapeHtml(target)}" type="button" title="Kopier komplett tilbud med CRM-ref, prislinjer og PDF-lenke.">Kopier tilbud</button>
          </div>
          <details class="inline-more-actions lead-offer-more-actions">
            <summary title="Vis malvalg, alternativ e-poståpning og manuell sendtmerking.">Mer</summary>
            <div>
              ...
              <button class="secondary" data-mark-offer-sent-manual="${escapeHtml(target)}" type="button" title="Bruk bare etter at tilbudet faktisk er sendt utenfor CRM. Lagrer historikk og flytter saken til venter svar.">Marker sendt manuelt</button>
            </div>
          </details>
        </div>
      </section>
  ...
  function generatedHeatPumpOfferText(target) {
    ...
    return [
      ...
      "",
      "Standard montering inkluderer normalt:",
      "- Rør, deksel og kabel inntil 4 meter mellom utedel og innedel",
      "- Plassering av utedel på vegg inntil ca. 1,5 meter over bakkenivå når vegg/feste er egnet",
      "- Boring i normal trevegg",
      "",
      "Avklares eller kommer i tillegg ved behov:",
      "- Elektrisk strømtilførsel til varmepumpen. Dette må utføres av autorisert elektriker.",
      "- Ekstra rør/kanal, spesialbraketter, murboring/kjerneboring, tak, dryppanne, varmepumpehus eller andre avvik fra standard montering.",
      "- Trevegg kan kreve annet veggfeste og fjærdempere.",
      "",
      "Alle installerte anlegg blir lagt inn i vårt serviceoppsett for videre oppfølging.",
      "",
      "Gi gjerne beskjed hvis du ønsker å gå videre, så finner vi en monteringsdato som passer.",
      "",
      "Mvh",
  ...
  function generateLeadOfferText(target) {
    ...
    try {
      ...
      fields.text.value = applyOfferDocumentSelection(text, Boolean(fields.priceList?.checked));
      renderOfferLinesTotal(target);
  ...
  function leadOfferPayload(target, options = {}) {
    ...
    const text = applyOfferDocumentSelection(textDraft, includePriceList);
    const offerAttachments = selectedOfferAttachments(includePriceList);
  ...
  function offerActivityDataFromPayload(payload = {}) {
    const lines = offerLinesFromText(payload.text || "");
    const asOptions = /(?:^|\n)Prisalternativer:/i.test(String(payload.text || ""));
    const product = leadStatusProductSummaryFromLines(lines)
      || offerMainProductSummary(lines || payload.text || "")
      || cleanLeadStatusProductPrefill(payload.subject || "")
      || "Tilbud";
    const total = asOptions ? 0 : jobPriceBasisTotal(lines || payload.text || "");
  ...
  async function copyRouteMessage(kind, customerId, button) {
    ...
    const copied = await copyTextToClipboard(text, { manualFallback: true });
    const oldText = button?.textContent || "";
    if (button) button.textContent = copied ? "Kopiert" : "Kopier manuelt";
    setSyncStatus(
      copied
        ? (kind === "email" ? "E-posttekst kopiert. Lim den inn i e-post foreløpig." : "SMS-tekst kopiert. Lim den inn i Google Messages foreløpig.")
        : manualCopyStatus("Teksten"),
      copied ? "ok" : "",
    );
    if (button) {
      setTimeout(() => {
  ...
  function renderInstallationList(customer, installations) {
    ...
    const fallback = [];
    const legacyFallback = legacyCustomerInstallationFallback(customer);
    if (!displayRows.length && !rows.length && legacyFallback) {
      fallback.push(legacyFallback);
    }
    const visible = displayRows.length ? displayRows : fallback;
    const key = customerKey(customer);
  ...
  function renderCustomerOpenLeads(customer) {
    ...
    ` : "";
    if (!entries.length) {
      return `<section class="detail-section"><div class="section-title-row"><h3>Oppfølging og tilbud</h3>${actions}</div><div class="empty-state">Ingen åpne oppfølginger eller tilbud på kunden.</div></section>`;
    }
    return `
      <section class="detail-section">
        ...
        <div class="customer-order-list">
          ${entries.map((entry) => {
            ...
            const note = leadNoteForEntry(entry);
            const existingLeadOrder = orderForLeadEntry(entry);
            const openCaseButton = `<button data-open-lead-entry="${escapeHtml(target)}" type="button" title="Åpne saken i Innboks med riktig status/filter valgt.">Åpne sak</button>`;
            const nextActionButtons = status === "won" ? (existingLeadOrder
              ...
              : `<button class="order-primary" data-create-order-from-lead="${escapeHtml(target)}" type="button">Opprett jobb</button>
                <details class="inline-more-actions"><summary>Mer</summary><div>
                  <button class="secondary" data-create-order-and-book-from-lead="${escapeHtml(target)}" type="button">Opprett og book tid</button>
                </div></details>`) : "";
            return `
              <article>
                <div>
  ...
  function activityTypeLabel(type) {
    ...
    if (value === "contact_call") return "Ring";
    if (value === "contact_sms") return "SMS";
    if (value === "contact_email") return "E-post";
    if (value === "contact_meeting") return "Møte / befaring";
  ...
  function planningDayTimeline(rows, dayIso = "") {
    ...
    const events = sorted.map((row) => {
      ...
      if (bookingNeedsInvoiceFinalAction(row)) statusBits.push("Utkast i eAccounting");
      ...
      return `
        ...
        >
          <strong>${timeRangeText(bookingRowStartMinutes(row), bookingRowEndMinutes(row))} · ${formatDuration(bookingDurationMinutes(row))}</strong>
  ...
  function jobCard(row, editable, options = {}) {
    ...
    const durationText = formatDuration(bookingDurationMinutes(row));
    const headerParts = options.showDate
      ? [weekdayDate(row.booking.date, { long: true }), `kl. ${bookingTimeText(row.booking)}`, durationText, row.booking.resource || "", bookingJobLabel(row)]
      : [bookingTimeText(row.booking), durationText, row.booking.resource || "", bookingJobLabel(row)];
    ...
    if (editable && isAdmin()) {
      if (!done && needsMove) {
        primaryActions.push(`<button class="order-primary" data-edit-booking="${escapeHtml(row.id)}" type="button" title="Velg nytt tidspunkt for jobben.">Flytt jobb</button>`);
      } else if (!done) {
        primaryActions.push(`<button class="order-primary" data-complete-booking="${escapeHtml(row.id)}" type="button" title="Marker jobben som utført og velg eventuell neste oppfølging.">Fullfør</button>`);
      } else if (bookingNeedsPaymentAction(row)) {
        primaryActions.push(`<button class="order-primary" data-billing-booking="${escapeHtml(row.id)}" type="button" title="${escapeHtml(paymentActionTitle)}">${escapeHtml(paymentActionLabel)}</button>`);
      } else if (canHavePriceBasis) {
        primaryActions.push(`<button class="order-primary" data-price-basis-booking="${escapeHtml(row.id)}" type="button" title="Rediger prisgrunnlag og tillegg før eller etter faktura.">Fakturagrunnlag</button>`);
      }
    }
    if (row.customer.phone) secondaryActions.push(`<a href="tel:${escapeHtml(phoneForLink(row.customer.phone))}" title="Ring kunden fra enhet som støtter telefonsamtaler.">Ring</a>`);
    if (mapQuery(row.customer)) secondaryActions.push(`<a href="${escapeHtml(mapsUrl(row.customer))}" target="_blank" rel="noreferrer" title="Åpne adressen eller koordinatene i Google Maps.">Kart</a>`);
    if (row.customer.phone) moreActions.push(copyPhoneButton(row.customer.phone, "Kopier nr"));
    moreActions.push(`<a href="${escapeHtml(googleCalendarUrl(row))}" target="_blank" rel="noreferrer" title="Åpne denne jobben som et ferdig kalenderforslag.">Kalender</a>`);
    if (editable && isAdmin()) {
      if (!(!done && needsMove)) {
        moreActions.push(`<button data-edit-booking="${escapeHtml(row.id)}" type="button" title="Endre tidspunkt, type, tekniker eller notat på denne jobben.">${needsMove ? "Flytt" : "Endre"}</button>`);
      }
      if (done && canHavePriceBasis && !bookingNeedsPaymentAction(row)) {
        moreActions.push(`<button data-price-basis-booking="${escapeHtml(row.id)}" type="button" title="Rediger prisgrunnlag og tillegg før eller etter faktura.">Fakturagrunnlag</button>`);
      }
      if (!done && !needsMove) {
        moreActions.push(`<button data-move-booking="${escapeHtml(row.id)}" type="button" title="Marker at jobben må flyttes fordi kunden ikke var tilgjengelig, dere ikke kom inn, eller tidspunkt må avtales på nytt.">Må flyttes</button>`);
      }
      moreActions.push(`<button data-delete-booking="${escapeHtml(row.id)}" type="button" title="Fjern avtalen fra planen uten å slette kundehistorikk.">Fjern avtale</button>`);
    }
    const moreActionsHtml = moreActions.length ? `
      <details class="inline-more-actions">
        <summary title="Vis kopier, kalender, endring og sjeldnere valg.">Mer</summary>
        <div>${moreActions.join("")}</div>
      </details>
    ` : "";
    card.innerHTML = `
      ...
      ${bookingNeedsInvoiceFinalAction(row) ? `<span class="invoice-pill draft">Utkast i eAccounting</span>` : ""}
  ...
  function shouldPreferLegacyClipboard() {
    try {
  ...
  function copyTextWithHiddenTextarea(value) {
  ...
  async function copyTextToClipboard(text, options = {}) {
    const value = String(text || "");
    const preferLegacy = shouldPreferLegacyClipboard();
    if (preferLegacy && copyTextWithHiddenTextarea(value)) {
      if (options.mobileAssist) showManualCopyFallback(value, { copied: true });
      return true;
    }
    if (navigator.clipboard?.writeText) {
      try {
        await navigator.clipboard.writeText(value);
        if (options.mobileAssist && preferLegacy) showManualCopyFallback(value, { copied: true });
        return true;
      } catch (_error) {
        // Flere mobilnettlesere eksponerer clipboard API, men nekter selve kopieringen.
      }
    }
    if (!preferLegacy && copyTextWithHiddenTextarea(value)) return true;
    showManualCopyFallback(value);
    if (options.manualFallback) return false;
    throw new Error("Klarte ikke kopiere automatisk. Teksten er markert i kopieringsboksen.");
  }

  function manualCopyStatus(label = "Teksten") {
    return `${label} ligger i kopieringsboksen. Hold inne og velg Kopier på mobilen.`;
  }

  async function copyTextWithStatus(text, copiedMessage, manualLabel = "Teksten") {
    const copied = await copyTextToClipboard(text, { manualFallback: true });
    setSyncStatus(copied ? copiedMessage : manualCopyStatus(manualLabel), copied ? "ok" : "");
    return copied;
  }

  function showManualCopyFallback(text, options = {}) {
    ...
    wrap.innerHTML = `
      <div>
        <strong>${copied ? "Kopiering klar" : "Kopier manuelt"}</strong>
      ...
      <p>${copied ? "Hvis teksten ikke kan limes inn, bruk feltet under som reserve." : "Hold inne i feltet og velg Kopier. Trykk Marker alt hvis mobilen mister markeringen."}</p>
      <textarea rows="4"></textarea>
    `;
    const textarea = wrap.querySelector("textarea");
    const close = wrap.querySelector("[aria-label='Lukk kopieringsboks']");
    const selectAll = wrap.querySelector("[data-select-manual-copy]");
    textarea.value = text;
  ...
  async function copyDayPlan() {
    ...
    const copied = await copyTextToClipboard(text, { manualFallback: true });
    setSyncStatus(copied ? `Dagsplan kopiert (${todaysPersonalRows().length} jobber).` : manualCopyStatus("Dagsplanen"), copied ? "ok" : "");
  }
  ...
  async function copyPhone(button) {
    ...
    if (!phone) return;
    const oldText = button.textContent;
    const copied = await copyTextToClipboard(phone, { manualFallback: true, mobileAssist: true });
    button.textContent = copied ? "Kopiert" : "Kopier manuelt";
    setSyncStatus(
      copied
        ? (shouldPreferLegacyClipboard() ? `Telefonnummer kopiert: ${phone}. Kopieringsboks ligger klar hvis mobilen ikke limer inn.` : `Telefonnummer kopiert: ${phone}`)
        : manualCopyStatus("Telefonnummeret"),
      copied ? "ok" : "",
    );
    setTimeout(() => {
      button.textContent = oldText;
  ...
  async function copyEmail(button) {
    ...
    if (!email) return;
    const oldText = button.textContent;
    const copied = await copyTextToClipboard(email, { manualFallback: true, mobileAssist: true });
    button.textContent = copied ? "Kopiert" : "Kopier manuelt";
    setSyncStatus(
      copied
        ? (shouldPreferLegacyClipboard() ? `E-post kopiert: ${email}. Kopieringsboks ligger klar hvis mobilen ikke limer inn.` : `E-post kopiert: ${email}`)
        : manualCopyStatus("E-postadressen"),
      copied ? "ok" : "",
    );
    setTimeout(() => {
      button.textContent = oldText;
  ...
  async function openSmsWithCopiedNumber(button) {
    const phone = button.dataset.openSmsPhone || "";
    if (!phone) return;
    const oldText = button.textContent;
    const targetUrl = smsUrl(phone);
    let openedWindow = null;
    if (!targetUrl.startsWith("sms:")) {
      openedWindow = window.open("about:blank", "_blank", "noopener,noreferrer");
    }
    const copied = await copyTextToClipboard(phone, { manualFallback: true, mobileAssist: true });
    button.textContent = copied ? "SMS klar" : "Kopier manuelt";
    setSyncStatus(
      copied
        ? `Telefonnummer kopiert: ${phone}. Lim inn i SMS/Google Messages hvis mottakerfeltet ikke fylles automatisk.`
        : `${manualCopyStatus("Telefonnummeret")} Lim inn nummeret i SMS/Google Messages.`,
      copied ? "ok" : "",
    );
    if (openedWindow) {
      openedWindow.location.href = targetUrl;
    } else if (targetUrl.startsWith("sms:")) {
  ...
  function snapshotBlockedKey(key) {
    return /password|passord|secret|access[_-]?token|refresh[_-]?token|id[_-]?token|api[_-]?key|authorization/i.test(String(key || ""));
  }

  function snapshotSafeValue(value, depth = 0) {
    ...
    if (Array.isArray(value)) return value.map((item) => snapshotSafeValue(item, depth + 1));
    if (typeof value === "object") {
      const output = {};
      for (const [key, item] of Object.entries(value)) {
        if (snapshotBlockedKey(key) || typeof item === "function") continue;
        output[key] = snapshotSafeValue(item, depth + 1);
      }
      return output;
    }
    return value;
  ...
  function snapshotObjectRows(objectRows) {
    return Object.entries(objectRows || {}).map(([id, row]) => snapshotSafeValue({
      id: row?.id || id,
      ...row,
    }));
  }

  function buildPrivateDataSnapshot() {
    const customerLocations = uniqueSnapshotRowsFromMap(customerLocationsByCustomer);
    const accessNotes = uniqueSnapshotRowsFromMap(accessNotesByCustomer);
    const installations = uniqueSnapshotRowsFromMap(installationsByCustomer);
    const invoices = uniqueSnapshotRowsFromMap(invoicesByCustomer);
    const serviceEvents = uniqueSnapshotRowsFromMap(serviceEventsByCustomer);
    const bookingRowsSnapshot = snapshotObjectRows(bookings);
    const orderRowsSnapshot = snapshotObjectRows(orders);
    const activeInboxCount = activeInboxSnapshotCount();
    return {
      schema: "numedal-crm-private-snapshot/v1",
      exported_at: new Date().toISOString(),
      app_version: "20260706-02",
      source: store.isConfigured ? "supabase" : "local-demo",
      counts: {
        ...
        bookings: bookingRowsSnapshot.length,
        orders: orderRowsSnapshot.length,
        jobs: jobs.length,
        appointments: appointments.length,
        leads: leads.length,
        websiteSubmissions: websiteSubmissions.length,
        intakeItems: intakeItems.length,
        activeInboxItems: activeInboxCount,
        crmAttachments: crmAttachments.length,
      },
      customers: snapshotSafeValue(customers),
      customerLocations: snapshotSafeValue(customerLocations),
      accessNotes: snapshotSafeValue(accessNotes),
      installations: snapshotSafeValue(installations),
      invoices: snapshotSafeValue(invoices),
      serviceEvents: snapshotSafeValue(serviceEvents),
      bookings: bookingRowsSnapshot,
      orders: orderRowsSnapshot,
      jobs: snapshotSafeValue(jobs),
      appointments: snapshotSafeValue(appointments),
      leads: snapshotSafeValue(leads),
      websiteSubmissions: snapshotSafeValue(websiteSubmissions),
      intakeItems: snapshotSafeValue(intakeItems),
      crmAttachments: snapshotSafeValue(crmAttachments),
      profiles: snapshotSafeValue((profiles || []).map((profile) => ({
        id: profile.id,
  ...
  function downloadPrivateDataSnapshot() {
    const snapshot = buildPrivateDataSnapshot();
    const text = JSON.stringify(snapshot, null, 2);
    downloadTextFile(`numedal-crm-private-snapshot-${isoDate(new Date())}.json`, text, "application/json;charset=utf-8");
    setSyncStatus(`Privat CRM-snapshot lastet ned (${snapshot.counts.customers.toLocaleString("nb-NO")} kunder, ${snapshot.counts.installations.toLocaleString("nb-NO")} anlegg).`, "ok");
  }
  ...
  function openCustomerQuickPanel(customerId, bookingId) {
    ...
    el.customerQuickContent.innerHTML = `
      ...
      <div class="quick-actions">
        ...
        ${bookingId && isAdmin() ? `<button class="secondary" data-edit-booking="${escapeHtml(bookingId)}" type="button">Endre tid</button>` : ""}
      </div>
    `;
  ...
  async function saveInstallationFromDialog() {
    ...
    if (store.isConfigured) {
      ...
      requireLocalDemoStorage();
      savedInstallation = { ...values, id: values.id || `installation-${Date.now()}`, customer_id: customerKey(customer), source: "local" };
    }
  ...
  function nearestFittingSlotForBooking(values, ignoreId = "", options = {}) {
    if (!values?.date || !values?.time) return null;
    ...
    const maxDistance = Number.isFinite(Number(options.maxDistanceMinutes)) ? Number(options.maxDistanceMinutes) : Infinity;
    let best = null;
    for (const slot of slots) {
      const latestStart = slot.end - duration;
      const start = Math.max(slot.start, Math.min(window.start, latestStart));
      const distance = Math.abs(start - window.start);
      if (distance > maxDistance) continue;
      const candidate = { slot, start, end: start + duration, distance, time: timeFromMinutes(start) };
  ...
  function renderBookingDayAgenda() {
    ...
    const selectedDurationText = formatDuration(selectedDuration);
    const previewText = `${timeRangeText(selectedWindow.start, selectedWindow.end)} · ${selectedDurationText} · ${previewValues.resource || "valgt ansatt"}`;
    ...
    const slotHtml = slots.length ? slots.map((slot) => `
      <button class="free-slot" data-booking-slot-time="${escapeHtml(timeFromMinutes(slot.start))}" type="button" title="Bruk ${escapeHtml(timeFromMinutes(slot.start))} som starttid for ${escapeHtml(selectedDurationText)}.">${escapeHtml(slotSuggestionText(slot, selectedDuration))}</button>
    `).join("") : `<span class="free-slot busy">Ingen ledige hull for ${escapeHtml(selectedDurationText)} på valgt ansatt</span>`;
  ...
  function priceBasisLineUnitPrice(line) {
    const match = String(line || "").match(/\b(?:x|à|a)\s*([\d\s]+(?:,\d{1,2})?)\s*(?:,-|kr|inkl|=)/i);
    return match ? parseJobPriceAmount(match[1]) : 0;
  }

  function priceBasisLineLabel(line) {
    return String(line || "")
      .replace(/^\s*-\s*/, "")
      .split(":")[0]
      .replace(/^\d{3,6}\s+/, "")
  ...
  function eaccountingItemForPriceBasisLine(line) {
    const label = normalizeMatch(priceBasisLineLabel(line));
    if (!label) return null;
    const unitPrice = priceBasisLineUnitPrice(line);
    const candidates = eaccountingCatalogItems().filter((item) => {
      const display = normalizeMatch(offerLineDisplayLabel(item));
      const product = normalizeMatch(eaccountingProductName(item));
      return label === display || label === product || label.includes(display) || display.includes(label);
    });
    if (!candidates.length) return null;
    if (unitPrice > 0) {
      const priceMatch = candidates.find((item) => Math.abs(Number(item.price || 0) - unitPrice) < 0.01);
      if (priceMatch) return priceMatch;
    }
    return candidates[0];
  }

  function billingDraftPriceBasisText(priceBasis) {
    return String(priceBasis || "")
      .split(/\r?\n/)
      .map((line) => {
        const item = eaccountingItemForPriceBasisLine(line);
        const cleanLine = String(line || "").replace(/^\s*-\s*/, "").trim();
        if (!item?.articleNo || cleanLine.startsWith(`${item.articleNo} `)) return line;
        return line.replace(/^(\s*-\s*)/, `$1${item.articleNo} `);
      })
      .join("\n")
      .trim();
  }

  function parseCustomPriceLineText(value) {
    const raw = String(value || "").trim();
    if (!raw) return { label: "", price: 0 };
    const match = raw.match(/(?:^|[\s:=/-])(?:pris\s*)?(?:kr\s*)?(\d[\d\s]*(?:,\d{1,2})?)\s*(?:,-|kr)?\s*$/i);
    if (!match) return { label: raw, price: 0 };
    const price = parseJobPriceAmount(match[1]);
    const hasPriceMarker = /(?:pris|kr|,-|=|:)\s*\d[\d\s]*(?:,\d{1,2})?\s*(?:,-|kr)?\s*$/i.test(raw);
    if (!price || (price < 10000 && !hasPriceMarker)) return { label: raw, price: 0 };
    const label = raw
      .slice(0, match.index)
      .replace(/[\s:=/-]+$/g, "")
      .trim();
  ...
  function priceLineForValues(label, quantity = 1, unit = "stk", price = 0) {
    const cleanLabel = String(label || "").trim();
    const cleanUnit = String(unit || "stk").trim() || "stk";
    const qty = Number(quantity || 1);
    const unitPrice = Number(price || 0);
    if (!cleanLabel || !Number.isFinite(qty) || qty <= 0) return "";
    if (!Number.isFinite(unitPrice) || unitPrice <= 0) {
      return `- ${cleanLabel}: ${formatJobPriceNumber(qty)} ${cleanUnit} - pris avtales/legges inn`;
    }
    return `- ${cleanLabel}: ${formatJobPriceNumber(qty)} ${cleanUnit} à ${formatJobPriceAmount(unitPrice)} = ${formatJobPriceAmount(qty * unitPrice)} inkl. mva`;
  }

  function customPriceLineFromText(value, quantity = 1, unit = "stk", explicitPrice = 0) {
    const parsed = parseCustomPriceLineText(value);
    const price = Number(explicitPrice || 0) > 0 ? Number(explicitPrice || 0) : parsed.price;
    return priceLineForValues(parsed.label || value, quantity, unit, price);
  }

  function jobPriceLineForItem(item, quantity) {
    return priceLineForValues(offerLineDisplayLabel(item), quantity || item.defaultQty || 1, item.unit, item.price);
  }
  ...
  function showCompletionDialogMessage(message, tone = "") {
    if (!el.completionDialogMessage) return;
    el.completionDialogMessage.textContent = message || "";
    el.completionDialogMessage.className = `dialog-message ${tone || ""}`.trim();
    el.completionDialogMessage.classList.toggle("hidden", !message);
  }

  function clearCompletionDialogMessage() {
    showCompletionDialogMessage("", "");
  }
  ...
  function syncBillingPriceQuantity() {
  ...
  function syncBillingPriceSearch() {
    if (!el.billingPricePreset) return;
    const query = el.billingPriceSearch?.value || "";
    const currentValue = el.billingPricePreset.value || "heatpump_custom";
    el.billingPricePreset.innerHTML = offerLinePresetOptions(currentValue, query);
    const available = new Set(Array.from(el.billingPricePreset.options).map((option) => option.value).filter(Boolean));
    const preferredValue = query.trim() && currentValue === "heatpump_custom"
      ? firstOfferLineMatch(query).id
      : currentValue;
    el.billingPricePreset.value = available.has(preferredValue) ? preferredValue : firstOfferLineMatch(query).id;
    syncBillingPriceQuantity();
  }

  function setupBillingPriceFields(row) {
    if (!el.billingPricePreset) return;
    if (el.billingPriceSearch) el.billingPriceSearch.value = "";
    const preferredId = defaultCompletionPriceItemId(bookingDisplayType(row)) || "standard_installation";
    el.billingPricePreset.innerHTML = offerLinePresetOptions(preferredId, "");
    if (preferredId && offerLinePresets.some((item) => item.id === preferredId)) {
      el.billingPricePreset.value = preferredId;
    }
    if (el.billingPriceQuantity) el.billingPriceQuantity.value = "1";
    syncBillingPriceQuantity();
  }

  function billingPriceLineFromSelection() {
    const selected = offerLinePresets.find((entry) => entry.id === el.billingPricePreset?.value);
    const quantity = Number(String(el.billingPriceQuantity?.value || selected?.defaultQty || 1).replace(",", "."));
    if (!Number.isFinite(quantity) || quantity <= 0) throw new Error("Antall må være større enn 0.");
    if (!selected || selected.id === "heatpump_custom") {
      const label = String(el.billingPriceSearch?.value || "").trim();
      if (!label) throw new Error("Søk eller velg produkt/tillegg før du legger til.");
      return customPriceLineFromText(label, quantity, "stk");
    }
    const line = jobPriceLineForItem(selected, quantity);
  ...
  function addBillingPriceLine() {
    if (!el.billingPriceBasis) return;
    try {
      const line = billingPriceLineFromSelection();
      const existing = String(el.billingPriceBasis.value || "").trim();
      if (offerLinesAlreadyInclude(existing, line)) {
        showBillingDialogMessage("Linjen ligger allerede i fakturagrunnlaget. Endre antall hvis kunden skal ha flere.", "info");
        return;
      }
      el.billingPriceBasis.value = [existing, line].filter(Boolean).join("\n");
      renderBillingPriceTotal();
      clearBillingDialogMessage();
    } catch (error) {
      showBillingDialogMessage(error.message || "Klarte ikke legge til linjen.", "error");
    }
  }
  ...
  function billingDraftTextForRow(row, options = {}) {
    ...
    const draftPriceBasis = billingDraftPriceBasisText(priceBasis);
    ...
    return [
      ...
      "",
      "Prislinjer:",
      draftPriceBasis || "Mangler prislinjer. Kontroller jobben før faktura lages.",
  ...
  async function copyBillingDraftFromDialog() {
    ...
    const copied = await copyTextToClipboard(text, { manualFallback: true });
    showBillingDialogMessage(
      copied
        ? "Fakturagrunnlag kopiert. Åpne eAccounting og lim det inn i fakturautkast/notat."
        : manualCopyStatus("Fakturagrunnlaget"),
      copied ? "ok" : "",
    );
    setSyncStatus(copied ? "Fakturagrunnlag kopiert. Lim det inn i eAccounting som grunnlag/notat før faktura sendes." : manualCopyStatus("Fakturagrunnlaget"), copied ? "ok" : "");
  }

  async function copyBillingDraftForBooking(id) {
    const row = bookingRows().find((item) => item.id === id);
    if (!row) throw new Error("Fant ikke jobben som skulle kopieres.");
    const text = billingDraftTextForRow(row);
    const copied = await copyTextToClipboard(text, { manualFallback: true });
  ...
  function addCompletionPriceLine() {
    ...
    if (!item || item.id === "heatpump_custom") {
      ...
      if (!Number.isFinite(quantity) || quantity <= 0) throw new Error("Antall må være større enn 0.");
      line = customPriceLineFromText(label, quantity, "stk");
    } else {
    ...
    if (offerLinesAlreadyInclude(existing, line)) {
      showCompletionDialogMessage("Linjen ligger allerede i prisgrunnlaget. Endre antall hvis kunden skal ha flere.", "info");
      return;
    }
    el.completionPriceLines.value = [existing, line].filter(Boolean).join("\n");
    renderCompletionPriceTotal();
    clearCompletionDialogMessage();
  }
  ...
  function setupCompletionInstallationField(row) {
    ...
    const installations = relevant
      ? selectableInstallationsForCustomer(row.customer, linkedInstallationId)
      : [];
    el.completionInstallation.innerHTML = "";
  ...
  async function saveActivityRecord(activity = {}) {
    const row = {
      ...
      location_id: activity.location_id || activity.locationId || null,
      installation_id: activity.installation_id || activity.installationId || null,
  ...
  async function createInstallationAfterCompletedInstallation(customer, row, doneDate, nextAction, nextServiceDate, priceBasis) {
    ...
    let savedInstallation;
    if (store.isConfigured) {
      if (!store.saveInstallation) throw new Error("Lagring av nytt anlegg krever oppdatert Supabase-adapter.");
  ...
  async function moveBookingToDate(id, targetDate, options = {}) {
    ...
    try {
      ...
      orderSyncWarning = error.message || "jobbkobling";
    }
    renderAll();
    ...
    const warningText = orderSyncWarning ? ` Booking er flyttet, men jobbkoblingen ble ikke oppdatert ferdig: ${orderSyncWarning}` : "";
  ...
  el.globalSearchResults?.addEventListener("pointerdown", (event) => {
    if (!activateGlobalSearchResult(event.target)) return;
    globalSearchPointerHandled = true;
    event.preventDefault();
    setTimeout(() => {
      globalSearchPointerHandled = false;
    }, 400);
  });
  el.globalSearchResults?.addEventListener("click", (event) => {
    if (globalSearchPointerHandled) return;
  ...
  el.leadStatusProduct?.addEventListener("input", renderLeadStatusProductTotal);
  ...
  function planningDropPlacementForDrop(event, bookingId, day) {
    const currentPlacement = planningDropPlacement(event, bookingId, day);
    const previewPlacement = planningDropPlacementFromPreview(bookingId, day);
    if (currentPlacement?.exactTimeDrop || currentPlacement?.fromTimelinePointer) return currentPlacement;
    return previewPlacement || currentPlacement;
  }
  ...
  el.planningBoard.addEventListener("dragover", (event) => {
    ...
    const slot = Array.from(day.querySelectorAll("[data-planning-time]"))
      ...
      || event.target.closest("[data-planning-time]");
  ...
  el.planningBoard.addEventListener("dragleave", (event) => {
    ...
    if (day && !day.contains(event.relatedTarget)) {
      ...
      if (planningDropPreview?.date === day.dataset.planningDate) planningDropPreview = null;
      delete day.dataset.planningDropTime;
      delete day.dataset.planningDropLabel;
      const hint = day.querySelector(".drop-hint");
