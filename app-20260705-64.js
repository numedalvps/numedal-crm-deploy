(function () {
  ...
  function renderCustomerDataCleanupWarning(customer) {
    ...
    const metadataGhostCount = installationsForCustomer(customer).filter((installation) => (
    ...
    if (metadataGhostCount) {
      parts.push(`
        <p>${metadataGhostCount.toLocaleString("nb-NO")} aktiv post i anleggslisten ser ut som gammel leadtekst, ikke ekte varmepumpe. Åpne anleggslisten under og arkiver feil-anleggene.</p>
      `);
    }
    return `
      <section class="detail-section attention compact-warning">
  ...
  function renderInstallationList(customer, installations) {
    ...
    const displayRows = rows.filter((installation) => !(installation.active === false && installationLooksLikeLeadMetadata(installation)));
    const fallback = [];
    ...
    ` : "";
    const archivedNotice = archivedMetadataCount ? `<div class="empty-state compact">${archivedMetadataCount.toLocaleString("nb-NO")} gammel leadtekst er skjult som arkivert feil-anlegg.</div>` : "";
    if (!visible.length) {
  ...
  function completionInstallationDraft(row, customer, doneDate, nextAction, nextServiceDate, priceBasis) {
    ...
    const fallbackProduct = cleanLeadStatusProductPrefill([customer?.brand, customer?.model_or_note].filter(Boolean).join(" ")) || cleanLeadStatusProductPrefill(text);
    const brand = item?.brand || brandFromPumpModel(fallbackProduct, text) || customer?.brand || "";
    let model = item?.label || fallbackProduct || customer?.model_or_note || "";
